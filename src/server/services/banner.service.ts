...
export const HOME_BANNER_PLACEMENTS = {
  HERO: 'HOME_HERO',
  PROMO_1: 'HOME_PROMO_1',
  PROMO_2: 'HOME_PROMO_2',
  MID: 'HOME_MID',
...
export function normalizeBannerPlacement(placement: string): BannerPlacement | string {
  return placementAliases[placement]?.[0] ?? placement;
}
...
export async function listActiveBanners(placement: string, limit = 12): Promise<BannerRow[]> {
  ...
  const rows = await prisma.$queryRawUnsafe<BannerRow[]>(
    `SELECT "id","key","placement","title","subtitle","ctaLabel","href","desktopImageUrl","mobileImageUrl","startAt","endAt","sortOrder","autoSlide","durationMs","isActive"
     FROM "Banner"
     WHERE "placement" IN (${placeholders}) AND "isActive" = true AND ${nowWindowSql()}
