...
export class ProductService {
  ...
  async listProducts(opts: ProductListOptions): Promise<ProductListResult> {
    const page = Math.max(1, opts.page ?? 1); const pageSize = Math.min(100, Math.max(1, opts.pageSize ?? 24)); const isSearch = Boolean(opts.q?.trim());
    const useSmartFeed = !isSearch && (!opts.sort || opts.sort === 'default' || opts.sort === 'recommended'); const useCandidateRanking = isSearch || useSmartFeed;
    const candidatePageSize = useCandidateRanking ? Math.min(200, Math.max(pageSize * 4, 100)) : pageSize; const logicalOffset = (page - 1) * pageSize;
    const candidatePage = useCandidateRanking ? Math.floor(logicalOffset / candidatePageSize) + 1 : page; const candidateOffset = useCandidateRanking ? logicalOffset % candidatePageSize : 0;
    const paginated = await this.products.findMany({ ...opts, page: candidatePage, pageSize: candidatePageSize, isActive: true }); const ratings = await this.products.getRatings(paginated.items.map((r) => r.id));
    const mapped = paginated.items.map((row) => { const rating = ratings.get(row.id); return { row, summary: mapProductSummary(row as never, { averageRating: rating?.average ?? 0, reviewCount: rating?.count ?? 0 }) }; }); let ranked: ProductSummary[];
    if (isSearch && opts.q && opts.rerank !== false) ranked = mapped.map(({ row, summary }) => ({ product: summary, score: computeSearchScore({ id: row.id, name: row.name, shortDescription: row.shortDescription, description: row.description, categoryName: row.category?.name, region: row.region, sellerShopName: row.seller?.sellerShopName, tags: readJsonArray(row.tagsJson) }, opts.q!) })).filter((x) => x.score > 0).sort((a, b) => b.score - a.score || (b.product.salesCount ?? 0) - (a.product.salesCount ?? 0) || a.product.id.localeCompare(b.product.id)).map((x) => x.product);
    else if (useSmartFeed) { ranked = rankProducts(mapped.map(({ summary }) => ({ ...summary, averageRating: summary.averageRating ?? 0, reviewCount: summary.reviewCount ?? 0, salesCount: summary.salesCount ?? 0, viewCount: summary.viewCount ?? 0, compareAtPrice: summary.comparePrice ?? null, createdAt: summary.createdAt ?? undefined })), DEFAULT_RANKING_CONFIG); ranked = diversifyProducts(ranked, { maxPerSeller: 3, maxPerCategory: 4 }); }
    else ranked = mapped.map(({ summary }) => summary);
    const start = useCandidateRanking ? candidateOffset : 0; const products = ranked.slice(start, start + pageSize); const hasMore = useCandidateRanking ? logicalOffset + products.length < paginated.total : paginated.hasMore;
  ...
  async createProduct(input: { slug: string; name: string; shortDescription: string; price: number; compareAtPrice?: number | null; categoryId: string; sellerId: string; region: string; currency?: string; inStock?: boolean; isActive?: boolean; stockQuantity?: number; description?: string | null; whatsappNumber?: string | null; videoUrl?: string | null; isTraditional?: boolean; imagesJson?: string | null; weightKg?: number | null; dimensionsJson?: string | null; tagsJson?: string | null; attributesJson?: string | null; primaryImageIndex?: number; }) {
    const category = await this.categories.findById(input.categoryId); if (!category) throw new ProductServiceError('category_not_found', 'دسته‌بندی انتخاب‌شده وجود ندارد. لطفاً یک دسته‌بندی معتبر انتخاب کنید.', 422);
  ...
  async getHomepageSections(sectionSize = 8): Promise<{ newest: ProductSummary[]; bestSelling: ProductSummary[]; mostViewed: ProductSummary[]; popular: ProductSummary[]; featured: ProductSummary[]; }> {
    const baseFilter: ProductListFilter = { isActive: true, pageSize: sectionSize }; const [newResult, bestResult, viewedResult, popularResult, featuredResult] = await Promise.all([
      this.products.findMany({ ...baseFilter, sort: 'newest', page: 1 }), this.products.findMany({ ...baseFilter, sort: 'bestSelling', page: 1 }), this.products.findMany({ ...baseFilter, sort: 'mostViewed', page: 1 }), this.products.findMany({ ...baseFilter, sort: 'popular', page: 1 }), this.products.findMany({ ...baseFilter, featured: true, sort: 'featured', page: 1 }),
    ]); return { newest: newResult.items.map((r) => mapProductSummary(r as never)), bestSelling: bestResult.items.map((r) => mapProductSummary(r as never)), mostViewed: viewedResult.items.map((r) => mapProductSummary(r as never)), popular: popularResult.items.map((r) => mapProductSummary(r as never)), featured: featuredResult.items.map((r) => mapProductSummary(r as never)) };
  }
}
