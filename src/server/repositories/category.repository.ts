...
function mapRow(row: RawCategory, withCount = true): CategoryRow {
  return {
    ...
    name: row.name,
    slug: row.slug,
    productCount: withCount ? Number(row.productCount ?? 0) : undefined,
...
export class PrismaCategoryRepository implements ICategoryRepository {
  ...
  private async queryOne(where: Prisma.Sql, withCount = true): Promise<CategoryRow | null> {
    const rows = await this.prisma.$queryRaw<RawCategory[]>(Prisma.sql`
      SELECT
        c."id", c."key", c."name", c."slug",
        m."parentId", p."key" AS "parentKey", m."imageUrl",
        COALESCE(m."isActive", true) AS "isActive",
        COALESCE(m."sortOrder", 0) AS "sortOrder",
      ...
      LEFT JOIN "Category" p ON p."id" = m."parentId"
      WHERE ${where}
      LIMIT 1
    `);
  ...
  async create(input: CreateCategoryInput): Promise<CategoryRow> {
    const row = await this.prisma.$transaction(async (tx) => {
      ...
      await tx.$executeRaw(Prisma.sql`
        INSERT INTO "CategoryMeta" ("categoryId", "parentId", "imageUrl", "isActive", "sortOrder")
        VALUES (${created.id}, ${input.parentId ?? null}, ${input.imageUrl ?? null}, ${input.isActive ?? true}, ${input.sortOrder ?? 0})
    ...
    if (!found) throw new Error('Created category could not be reloaded');
    return found;
  }
  ...
  async update(id: string, input: UpdateCategoryInput): Promise<CategoryRow> {
    await this.prisma.$transaction(async (tx) => {
      ...
      const current = await tx.$queryRaw<RawCategoryMeta[]>(Prisma.sql`
        SELECT "parentId", "imageUrl", "isActive", "sortOrder"
        FROM "CategoryMeta"
        WHERE "categoryId" = ${id}
        LIMIT 1
      `);
      ...
      await tx.$executeRaw(Prisma.sql`
        INSERT INTO "CategoryMeta" ("categoryId", "parentId", "imageUrl", "isActive", "sortOrder")
        VALUES (${id}, ${parentId}, ${imageUrl}, ${isActive}, ${sortOrder})
        ON CONFLICT ("categoryId") DO UPDATE SET
          "parentId" = EXCLUDED."parentId",
          "imageUrl" = EXCLUDED."imageUrl",
          "isActive" = EXCLUDED."isActive",
    ...
    if (!found) throw new Error('Updated category could not be reloaded');
    return found;
  }
