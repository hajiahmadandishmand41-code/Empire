 ...
 * Implements:
 *   - CRUD operations
 *   - Advanced filtering (price, stock, category, seller, isTraditional)
 *   - Smart ordering via buildProductOrderBy
 *   - Batch rating fetch (single query for a page of products)
 *   - Visibility rules (isActive gate)
 */
...
function jsonValue(raw: string | null | undefined): Prisma.InputJsonValue | null {
  if (raw == null || raw.trim() === '') return null;
  try { return JSON.parse(raw) as Prisma.InputJsonValue; }
  catch { throw new Error('Invalid JSON product field'); }
}
...
/** Prisma include shape for public product listings. */
const PRODUCT_LIST_INCLUDE = {
  category: true,
  seller: {
    select: { id: true, sellerShopName: true, sellerWhatsapp: true },
  },
} satisfies Prisma.ProductInclude;

/** Prisma include shape for detailed product view. */
...
// These types mirror the Prisma result shapes — they are domain-agnostic raw DB rows.
// Mappers in db-mappers.ts convert them to the typed domain models in src/types/.
...
export class PrismaProductRepository implements IProductRepository {
  ...
  async findSearchCandidates(
    ...
    const [rows, total] = await Promise.all([
      this.prisma.product.findMany({
        where,
        include: PRODUCT_LIST_INCLUDE,
        orderBy: [{ inStock: 'desc' }, { salesCount: 'desc' }, { createdAt: 'desc' }],
        take,
        skip,
      }),
      this.prisma.product.count({ where }),
  ...
  async create(input: CreateProductInput): Promise<ProductDetailRow> {
    const row = await this.prisma.product.create({
      data: {
        ...
        isTraditional: input.isTraditional ?? false,
        imagesJson: jsonValue(input.imagesJson),
        featuresJson: jsonValue(input.featuresJson),
        badge: input.badge ?? (input.compareAtPrice != null ? 'sale' : null),
        weightKg: input.weightKg ?? null,
        dimensionsJson: jsonValue(input.dimensionsJson),
        tagsJson: jsonValue(input.tagsJson),
        attributesJson: jsonValue(input.attributesJson),
  ...
  async update(id: string, input: UpdateProductInput): Promise<ProductDetailRow> {
    ...
    // Auto-sync badge when compareAtPrice changes
    if (input.compareAtPrice !== undefined) {
      data.badge = input.compareAtPrice != null ? 'sale' : null;
    }

    // Auto-sync inStock when stockQuantity changes
    if (typeof input.stockQuantity === 'number' && input.inStock === undefined) {
      data.inStock = input.stockQuantity > 0;
    }
  ...
  async countByCategory(categoryId: string): Promise<number> {
    return this.prisma.product.count({ where: { categoryId, isActive: true } });
  }
  ...
  private buildWhereClause(filter: ProductListFilter): Prisma.ProductWhereInput {
    const base: Prisma.ProductWhereInput = {
      isActive: filter.isActive !== false, // default to active only
    };

    if (filter.inStock !== undefined) base.inStock = filter.inStock;

    if (filter.isTraditional !== undefined) {
      (base as Record<string, unknown>).isTraditional = filter.isTraditional;
    }

    if (filter.categoryKey) {
      base.category = { key: filter.categoryKey };
    ...
    // Featured = has a discount OR has sold at least once
    if (filter.featured) {
      base.OR = [
    ...
    // Text search — uses both DB-level WHERE and in-memory re-ranking
    if (filter.q) {
      const searchWhere = buildSearchWhereClause(filter.q);
      return { ...base, ...searchWhere };
    }
