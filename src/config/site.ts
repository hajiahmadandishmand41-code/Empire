...
/** Eshop — central storefront configuration. */
function resolveSiteUrl(): string {
  const configured = process.env.NEXT_PUBLIC_SITE_URL;
  const runtimeDomain = (process.env.VERCEL_PROJECT_PRODUCTION_URL || process.env.VERCEL_URL)?.trim();
  const raw = configured || (runtimeDomain ? `https://${runtimeDomain}` : '');
  const trimmed = raw.replace(/\/$/, '');
  const isBuildPhase = process.env.NEXT_PHASE === 'phase-production-build';
  if (process.env.NODE_ENV === 'production' && !isBuildPhase && configured && (!trimmed || trimmed.startsWith('http://localhost'))) {
    throw new Error('[eshop] FATAL: NEXT_PUBLIC_SITE_URL must be a production URL.');
  }
