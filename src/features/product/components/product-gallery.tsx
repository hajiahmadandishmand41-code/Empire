  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div className="group relative aspect-square w-full overflow-hidden rounded-2xl border border-border bg-muted/20 shadow-sm">
        {current.src ? (
          <Image src={current.src!} alt={current.alt || productName} fill sizes="(max-width: 1024px) 100vw, 50vw" className="object-contain p-4 sm:p-8 transition-transform duration-300 group-hover:scale-105" />
        ) : (
      ...
      <div className="grid grid-cols-4 gap-2.5">
        {items.map((item, index) => {
          ...
          return (
            ...
            >
              {item.src ? (
                <Image src={item.src!} alt={item.alt || productName} fill sizes="(max-width: 768px) 25vw, 80px" className="object-cover" />
              ) : (
