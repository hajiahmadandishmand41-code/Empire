...
export function HomeDiscoveryStrip({ locale }: { locale: Locale }) {
  const copy = locale === 'en'
    ? { items: [
        ...
        { href: '/shop?weighted=true', label: 'Weighted Products', icon: Scale },
    ...
    : locale === 'ps'
      ? { items: [
          ...
          { href: '/shop?weighted=true', label: 'وزني محصولات', icon: Scale },
      ...
      : { items: [
          ...
          { href: '/shop?weighted=true', label: 'محصولات وزنی', icon: Scale },
          { href: '/discover', label: 'کشف', icon: Compass },
        ] };
      ...
      <div className="mx-auto max-w-screen-xl px-3 py-1.5 sm:px-6 sm:py-2">
        <div className="flex items-center justify-start gap-1.5 overflow-x-auto pb-0.5 no-scrollbar snap-x snap-mandatory sm:justify-center sm:gap-2.5 text-[10px] font-bold text-muted-foreground sm:text-xs">
          {copy.items.map(({ href, label, icon: Icon }) => (
            <Link key={href} href={href as never} className="group inline-flex shrink-0 snap-start items-center gap-1.5 whitespace-nowrap rounded-full border border-border/80 bg-background/80 px-3 py-1.5 transition-[transform,border-color,background-color,box-shadow] duration-200 hover:-translate-y-px hover:border-primary/30 hover:bg-primary/[0.03] hover:shadow-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary">
