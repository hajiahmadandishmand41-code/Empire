  ...
  const copy = locale === 'en'
    ? {
        ...
        items: [
          ...
          { href: '/discover#stores', label: 'Explore stores', icon: Store },
    ...
    : locale === 'ps'
      ? {
          ...
          items: [
            ...
            { href: '/discover#stores', label: 'پلورنځي وپلټئ', icon: Store },
      ...
      : {
          ...
          items: [
            ...
            { href: '/discover#stores', label: 'کشف فروشگاه‌ها', icon: Store },
    ...
    <section aria-label={copy.eyebrow} className="border-b border-border bg-card/90">
      <div className="mx-auto max-w-screen-xl px-3 py-2 sm:px-6 sm:py-2.5">
        <div className="flex items-center gap-2 overflow-x-auto pb-0.5 no-scrollbar snap-x snap-mandatory sm:justify-center sm:gap-4 text-[10px] font-bold text-muted-foreground sm:text-xs">
          <span className="hidden shrink-0 snap-start items-center gap-1.5 text-primary sm:inline-flex">
          ...
          {copy.items.map(({ href, label, icon: Icon }) => (
            <Link
              key={href}
              href={href as never}
              className="group inline-flex shrink-0 snap-start items-center gap-1.5 whitespace-nowrap rounded-full border border-border/80 bg-background/80 px-2.5 py-1.5 transition-[transform,border-color,background-color] duration-200 hover:-translate-y-px hover:border-primary/30 hover:bg-primary/[0.03] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary"
            >
