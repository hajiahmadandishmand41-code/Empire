...
export async function CategoryCard({ item }: CategoryCardProps) {
  const [t, locale] = await Promise.all([getTranslations('home.categories.items'), getLocale()]);
  ...
  return (
    <Link href={`/category/${item.slug}` as never} aria-label={title} className="group relative h-[118px] w-[118px] shrink-0 overflow-hidden rounded-2xl border border-border/70 bg-card shadow-sm transition-[transform,box-shadow,border-color] duration-200 hover:-translate-y-0.5 hover:border-primary/30 hover:shadow-premium focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary sm:h-[132px] sm:w-auto sm:shrink">
      ...
      <div className="absolute inset-x-0 bottom-0 p-2.5 text-white"><span className="line-clamp-2 text-[10px] font-black leading-4 drop-shadow-sm sm:text-[11px]">{title}</span>{productCount > 0 ? <span className="mt-0.5 block text-[8px] font-medium text-white/75 sm:text-[9px]">{productCount.toLocaleString(numberLocale)} {productLabel}</span> : null}</div>
    </Link>
  );
}
