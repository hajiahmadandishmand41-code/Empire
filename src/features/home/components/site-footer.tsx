...
import { Phone, Mail, MapPin, Instagram, Twitter, Send, Youtube, ChevronLeft, ShieldCheck, Truck, RotateCcw, CreditCard, Headphones } from 'lucide-react';
...
function FooterNav({ title, links }: { title: string; links: { label: string; href: string }[] }) {
  return <nav aria-label={title} className="space-y-4"><h3 className="flex items-center gap-2 text-xs font-extrabold uppercase tracking-wider text-foreground"><span className="inline-block h-3 w-0.5 rounded-full bg-primary" aria-hidden="true" />{title}</h3><ul className="space-y-2.5">{links.map((link) => <FooterLink key={`${link.href}-${link.label}`} {...link} />)}</ul></nav>;
}
...
export function SiteFooter() {
  ...
  const trustItems = [
    ...
    { icon: RotateCcw, label: t('trustReturns'), sub: t('trustReturnsSub'), iconBg: 'bg-amber-500/15', iconColor: 'text-amber-400' },
    { icon: CreditCard, label: t('trustPayment'), sub: t('trustPaymentSub'), iconBg: 'bg-rose-500/15', iconColor: 'text-rose-400' },
  ];
  const socials = [
    { icon: Instagram, href: '#', label: 'Instagram', className: 'hover:border-pink-500 hover:bg-pink-500/10 hover:text-pink-400' },
    { icon: Twitter, href: '#', label: 'Twitter', className: 'hover:border-sky-500 hover:bg-sky-500/10 hover:text-sky-400' },
    { icon: Send, href: '#', label: 'Telegram', className: 'hover:border-blue-500 hover:bg-blue-500/10 hover:text-blue-400' },
  ...
  return <footer className="border-t border-border bg-card" role="contentinfo">
    <div className="border-b border-border/60 py-4"><Container size="xl"><div className="grid grid-cols-2 gap-3 sm:grid-cols-4">{trustItems.map(({ icon: Icon, label, sub, iconBg, iconColor }) => <div key={label} className="group flex min-w-0 items-center gap-3 rounded-xl p-2 transition-colors hover:bg-muted/50"><div className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-xl ${iconBg} ring-1 ring-border`}><Icon className={`h-5 w-5 ${iconColor}`} aria-hidden="true" /></div><div className="min-w-0"><p className="truncate text-xs font-bold text-foreground group-hover:text-primary">{label}</p><p className="mt-0.5 text-[10px] text-muted-foreground">{sub}</p></div></div>)}</div></Container></div>
