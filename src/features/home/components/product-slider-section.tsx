...
export interface SliderProduct {
  id: string; name: string; slug: string; price: number; comparePrice?: number | null; images?: Array<{ url: string }>;
  badge?: string; rating?: number; reviewCount?: number; salesCount?: number; viewCount?: number;
  category?: { name: string }; sellerId?: string | null; sellerShopName?: string | null; sellerWhatsapp?: string;
}
...
function SliderProductCard({ product, locale = 'fa', currency = 'AFN' }: { product: SliderProduct; locale?: string; currency?: string }) {
  ...
  return <article className="group w-[160px] flex-none overflow-hidden rounded-2xl border border-border/70 bg-card shadow-sm transition-all hover:-translate-y-1 hover:shadow-lg sm:w-[180px] md:w-[200px]">
    ...
    <div className="flex flex-col gap-1.5 p-3">
      ...
      {(product.rating ?? 0) > 0 && <div className="flex items-center gap-0.5">{[0,1,2,3,4].map((i) => <Star key={i} className={cn('h-2.5 w-2.5', i < Math.round(product.rating!) ? 'fill-amber-400 text-amber-400' : 'text-muted-foreground')} />)}</div>}
      <div className="mt-1 flex items-center justify-between border-t border-border/60 pt-2"><span className="text-xs font-extrabold text-foreground">{formatPrice(product.price, currency, locale)}</span><Link href={`/shop/${product.slug}` as never} className="flex h-7 items-center gap-1 rounded-lg bg-rose-600 px-2.5 text-[10px] font-bold text-white"><ShoppingCart className="h-3 w-3" />{locale === 'en' ? 'Buy' : locale === 'ps' ? 'پېرود' : 'خرید'}</Link></div>
    </div>
  </article>;
}
...
export function ProductSliderSection({ title, subtitle, viewAllHref, accentColor = 'bg-rose-600', products, locale = 'fa', currency = 'AFN', skeleton = false }: ProductSliderSectionProps) {
  ...
  return <section className="border-b border-border bg-background py-6 sm:py-8" aria-label={title}>
    <div className="mx-auto max-w-screen-xl px-3 sm:px-6">
      ...
      <div ref={trackRef} className="flex gap-3 overflow-x-auto pb-2 no-scrollbar" dir="rtl">{skeleton ? Array.from({ length: 6 }).map((_, i) => <SkeletonCard key={i} />) : products.map((p) => <SliderProductCard key={p.id} product={p} locale={locale} currency={currency} />)}</div>
    </div>
  </section>;
}
