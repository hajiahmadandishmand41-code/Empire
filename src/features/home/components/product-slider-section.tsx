...
import { ChevronLeft, ChevronRight, Package, ShoppingCart, Plus, Star } from 'lucide-react';
...
export function SkeletonCard() { return <div className="w-[168px] flex-none overflow-hidden rounded-2xl border border-border bg-card sm:w-[188px] md:w-[204px]" aria-hidden="true"><div className="aspect-square animate-pulse bg-muted" /><div className="space-y-2 p-3"><div className="h-2 w-16 animate-pulse rounded bg-muted" /><div className="h-3 w-full animate-pulse rounded bg-muted" /><div className="h-3 w-3/4 animate-pulse rounded bg-muted" /><div className="h-7 w-20 animate-pulse rounded bg-muted" /></div></div>; }
...
function SliderProductCard({ product, locale = 'fa', currency = 'AFN' }: { product: SliderProduct; locale?: string; currency?: string }) {
  ...
  const canQuickAdd = Boolean(product.categoryKey && product.region);

  function quickAdd() {
    if (!canQuickAdd || adding || product.inStock === false) return;
    setAdding(true);
    useCartStore.getState().addItem({ slug: product.slug, name: product.name, price: product.price, region: product.region ?? 'افغانستان', categoryKey: product.categoryKey!, images: (product.images ?? []).map((item) => ({ src: item.url, alt: product.name })), quantity: 1 });
  ...
  return <article className="card-luxury group w-[168px] flex-none overflow-hidden rounded-2xl border border-border/80 bg-card sm:w-[188px] md:w-[204px]">
    <div className="relative">
      <Link href={`/shop/${product.slug}` as never} className="relative block aspect-square overflow-hidden bg-muted">
        {image ? <Image src={image} alt={product.name} fill sizes="(max-width: 640px) 168px, (max-width: 1024px) 188px, 204px" loading="lazy" className="object-cover transition-transform duration-500 ease-out group-hover:scale-[1.025]" /> : <div className="flex h-full items-center justify-center"><Package className="h-10 w-10 text-muted-foreground/30" aria-hidden /></div>}
      ...
      <WishlistButton slug={product.slug} productId={product.id} size="sm" labelOn={locale === 'en' ? 'Remove from wishlist' : locale === 'ps' ? 'له خوښو لرې کول' : 'حذف از علاقه‌مندی‌ها'} labelOff={locale === 'en' ? 'Add to wishlist' : locale === 'ps' ? 'خوښو ته اضافه کول' : 'افزودن به علاقه‌مندی‌ها'} className="absolute end-2 top-2 z-10 border-white/70 bg-white/90 dark:border-border dark:bg-gray-900/90" />
    </div>
    <div className="flex flex-col gap-1.5 p-3">
      ...
      {product.sellerShopName && product.sellerId && <Link href={`/store/${product.sellerId}` as never} className="truncate text-[10px] text-muted-foreground hover:text-primary">{product.sellerShopName}</Link>}
      {(product.rating ?? 0) > 0 && <div className="flex items-center gap-1" aria-label={`${product.rating}/5`}>{[0,1,2,3,4].map((i) => <Star key={i} className={cn('h-2.5 w-2.5', i < Math.round(product.rating!) ? 'fill-price-warning text-price-warning' : 'text-muted-foreground/30')} aria-hidden="true" />)}{product.reviewCount ? <span className="text-[9px] text-muted-foreground">({product.reviewCount})</span> : null}</div>}
      <div className="mt-1 border-t border-border/60 pt-2.5">{discountPct > 0 && product.comparePrice && <span className="block text-[10px] text-muted-foreground line-through">{formatPrice(product.comparePrice, currency, locale)}</span>}<div className="flex items-end justify-between gap-2"><div className="min-w-0"><span className="block truncate text-sm font-black text-price-current">{formatPrice(product.price, currency, locale)}</span>{product.salesCount ? <span className="text-[9px] text-muted-foreground">{product.salesCount.toLocaleString(locale === 'en' ? 'en-US' : 'fa-AF')} {salesLabel}</span> : null}</div>{canQuickAdd && product.inStock !== false ? <button type="button" onClick={quickAdd} aria-label={locale === 'en' ? 'Quick add' : locale === 'ps' ? 'چټک اضافه' : 'افزودن سریع'} className="flex h-8 shrink-0 items-center gap-1 rounded-lg bg-primary px-2.5 text-[10px] font-bold text-primary-foreground shadow-sm transition hover:-translate-y-0.5 hover:shadow-md disabled:opacity-70" disabled={adding}>{adding ? '✓' : <><Plus className="h-3 w-3" />{locale === 'en' ? 'Add' : locale === 'ps' ? 'اضافه' : 'افزودن'}</>}</button> : <Link href={`/shop/${product.slug}` as never} className="flex h-8 shrink-0 items-center gap-1 rounded-lg bg-primary px-2.5 text-[10px] font-bold text-primary-foreground"><ShoppingCart className="h-3 w-3" />{locale === 'en' ? 'View' : locale === 'ps' ? 'کتل' : 'مشاهده'}</Link>}</div></div>
    </div>
  </article>;
}
