...
export function SkeletonCard() {
  return <div className="w-[calc((100vw-2.5rem)/3)] max-w-[204px] flex-none overflow-hidden rounded-2xl border border-olive-300/60 bg-white/90 shadow-sm dark:border-slate-800 dark:bg-slate-900/90" aria-hidden="true"><div className="aspect-[3/4] animate-pulse bg-lime-100 dark:bg-slate-800" /><div className="space-y-2 p-2.5 sm:p-3"><div className="h-2 w-16 animate-pulse rounded bg-muted" /><div className="h-3 w-full animate-pulse rounded bg-muted" /><div className="h-3 w-3/4 animate-pulse rounded bg-muted" /><div className="h-7 w-20 animate-pulse rounded bg-muted" /></div></div>;
}
...
export function ProductSliderSection({ title, subtitle, viewAllHref, accentColor = 'bg-rose-600', products, locale = 'fa', currency = 'AFN', skeleton = false }: ProductSliderSectionProps) {
  ...
  return <section className="border-y border-lime-900/20 bg-gradient-to-r from-lime-100 via-emerald-50 to-lime-200 py-4 sm:py-6 dark:border-lime-200/10 dark:from-[#182018] dark:via-[#223022] dark:to-[#172117]" aria-label={title || undefined}>
    <div className="mx-auto max-w-screen-xl px-3 sm:px-6">
      <div className="mb-3 flex items-center justify-between gap-3">
        <div className="flex min-w-0 items-center gap-2">{title && <span className={cn('h-6 w-1 rounded-full', accentColor)} />}<div className="min-w-0">{title && <h2 className="truncate text-sm font-black tracking-tight text-foreground">{title}</h2>}{subtitle && <p className="mt-0.5 line-clamp-1 text-[10px] text-muted-foreground sm:text-[11px]">{subtitle}</p>}</div></div>
        <div className="flex shrink-0 items-center gap-1.5"><button type="button" onClick={() => scroll(-1)} aria-label={locale === 'en' ? 'Previous products' : locale === 'ps' ? 'مخکني محصولات' : 'محصولات قبلی'} className="flex h-8 w-8 items-center justify-center rounded-full border border-lime-900/15 bg-white/85 text-muted-foreground shadow-sm transition-colors hover:bg-white hover:text-foreground dark:border-white/10 dark:bg-slate-900/80"><ChevronRight className="h-4 w-4 rtl:rotate-180" /></button><button type="button" onClick={() => scroll(1)} aria-label={locale === 'en' ? 'Next products' : locale === 'ps' ? 'راتلونکي محصولات' : 'محصولات بعدی'} className="flex h-8 w-8 items-center justify-center rounded-full border border-lime-900/15 bg-white/85 text-muted-foreground shadow-sm transition-colors hover:bg-white hover:text-foreground dark:border-white/10 dark:bg-slate-900/80"><ChevronLeft className="h-4 w-4 rtl:rotate-180" /></button>{viewAllHref && <Link href={viewAllHref as never} className="rounded-full border border-lime-900/15 bg-white/85 px-3 py-1.5 text-[10px] font-bold shadow-sm transition-colors hover:bg-white sm:text-[11px] dark:border-white/10 dark:bg-slate-900/80">{allLabel}</Link>}</div>
      </div>
      <div ref={trackRef} className="flex snap-x snap-mandatory gap-2.5 overflow-x-auto pb-1 no-scrollbar" dir={locale === 'en' ? 'ltr' : 'rtl'}>{skeleton ? Array.from({ length: 6 }).map((_, i) => <SkeletonCard key={i} />) : products.map((product) => <div key={product.id} className="snap-start"><MarketplaceProductCard product={toProductSummary(product)} locale={locale} currency={currency} view="rail" /></div>)}</div>
    </div>
  </section>;
}
