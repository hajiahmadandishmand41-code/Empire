...
export function SkeletonCard() {
  return <div className="w-[calc((100vw-2.5rem)/3)] max-w-[204px] flex-none overflow-hidden rounded-2xl border border-stone-300/70 bg-white/75 shadow-sm dark:border-slate-800 dark:bg-slate-900/80 sm:w-[188px] md:w-[204px]" aria-hidden="true"><div className="aspect-[3/4] animate-pulse bg-stone-200/80 dark:bg-slate-800" /><div className="space-y-2 p-2.5 sm:p-3"><div className="h-2 w-16 animate-pulse rounded bg-muted" /><div className="h-3 w-full animate-pulse rounded bg-muted" /><div className="h-3 w-3/4 animate-pulse rounded bg-muted" /><div className="h-7 w-20 animate-pulse rounded-full bg-muted" /></div></div>;
}
...
export function ProductSliderSection({ title, subtitle, viewAllHref, accentColor = 'bg-rose-600', products, locale = 'fa', currency = 'AFN', skeleton = false }: ProductSliderSectionProps) {
  ...
  return <section className="border-y border-stone-300/70 bg-gradient-to-r from-stone-100 via-amber-50/80 to-stone-200/75 py-4 sm:py-6 dark:border-slate-800 dark:from-slate-950 dark:via-rose-950/20 dark:to-slate-900" aria-label={title || undefined}>
    <div className="mx-auto max-w-screen-xl px-3 sm:px-6">
      <div className="mb-3 flex items-center justify-between gap-3">
        <div className="flex min-w-0 items-center gap-2">{title && <span className={cn('h-6 w-1 rounded-full', accentColor)} />}<div className="min-w-0">{title && <h2 className="truncate text-sm font-black tracking-tight">{title}</h2>}{subtitle && <p className="mt-0.5 line-clamp-1 text-[10px] text-muted-foreground sm:text-[11px]">{subtitle}</p>}</div></div>
        <div className="flex shrink-0 items-center gap-1.5"><button type="button" onClick={() => scroll(-1)} aria-label={locale === 'en' ? 'Previous products' : locale === 'ps' ? 'مخکني محصولات' : 'محصولات قبلی'} className="flex h-8 w-8 items-center justify-center rounded-full border border-stone-300 bg-white/75 text-muted-foreground shadow-sm transition-colors hover:bg-white hover:text-foreground dark:border-slate-700 dark:bg-slate-900/80"><ChevronRight className="h-4 w-4 rtl:rotate-180" /></button><button type="button" onClick={() => scroll(1)} aria-label={locale === 'en' ? 'Next products' : locale === 'ps' ? 'راتلونکي محصولات' : 'محصولات بعدی'} className="flex h-8 w-8 items-center justify-center rounded-full border border-stone-300 bg-white/75 text-muted-foreground shadow-sm transition-colors hover:bg-white hover:text-foreground dark:border-slate-700 dark:bg-slate-900/80"><ChevronLeft className="h-4 w-4 rtl:rotate-180" /></button>{viewAllHref && <Link href={viewAllHref as never} className="rounded-full border border-stone-300 bg-white/75 px-3 py-1.5 text-[10px] font-bold shadow-sm transition-colors hover:bg-white sm:text-[11px] dark:border-slate-700 dark:bg-slate-900/80">{allLabel}</Link>}</div>
      </div>
