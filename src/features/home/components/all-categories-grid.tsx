...
export function AllCategoriesGrid({ items, locale, placeholder }: { items: Item[]; locale: string; placeholder: string }) {
  ...
  return <div className="grid gap-6 lg:grid-cols-[220px_minmax(0,1fr)] lg:items-start lg:[direction:ltr]">
    <aside className="lg:sticky lg:top-24" dir={textDir}>
      <div className="rounded-2xl border border-border bg-card p-3 shadow-sm">
        <label className="relative block"><span className="sr-only">{placeholder}</span><Search className="pointer-events-none absolute start-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" aria-hidden="true" /><input value={query} onChange={(e) => setQuery(e.target.value)} type="search" placeholder={placeholder} className="h-11 w-full rounded-xl border border-border bg-background ps-9 pe-3 text-sm outline-none transition-colors focus:border-primary" /></label>
        <div className="mt-3 space-y-1">
          <button type="button" onClick={() => setActive('all')} className={`flex w-full items-center justify-between rounded-xl px-3 py-2.5 text-start text-xs font-extrabold transition-colors ${active === 'all' ? 'bg-primary text-primary-foreground' : 'text-foreground hover:bg-muted'}`}><span>{all}</span><span>{items.length}</span></button>
    ...
    <div dir={textDir}>
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
        {filtered.map((item) => { const Icon = icons[item.iconKey]; return <Link key={item.key} href={item.href as never} className="group overflow-hidden rounded-2xl border border-border bg-card shadow-sm transition-all duration-200 hover:-translate-y-1 hover:border-primary/25 hover:shadow-lg">
          <div className="relative aspect-[1.35/1] overflow-hidden bg-muted"><div className="absolute inset-0 bg-cover bg-center transition-transform duration-500 group-hover:scale-105" style={{ backgroundImage: `url(${images[item.key]})` }} aria-hidden="true" /><div className="absolute inset-0 bg-gradient-to-t from-black/75 via-black/15 to-transparent" aria-hidden="true" /><div className="absolute inset-x-0 bottom-0 flex items-end justify-between gap-3 p-4 text-white"><div className="min-w-0"><div className="flex items-center gap-2"><span className="flex h-9 w-9 items-center justify-center rounded-xl bg-white/15 backdrop-blur-md"><Icon className="h-4 w-4" aria-hidden="true" /></span><h2 className="truncate text-sm font-black sm:text-base">{label(item)}</h2></div><span className="mt-2 inline-block text-[10px] text-white/75">{browse}</span></div><span className="shrink-0 rounded-xl bg-white px-3 py-2 text-[10px] font-extrabold text-primary shadow-sm">→</span></div></div>
        </Link>; })}
      </div>
