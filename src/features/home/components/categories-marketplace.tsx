...
export function CategoriesMarketplace({ categories, locale, allLabel }: Props) {
  ...
  return <div className="grid gap-5 lg:grid-cols-[230px_minmax(0,1fr)]">
    <aside className="lg:sticky lg:top-24 lg:self-start"><div className="rounded-3xl border border-border bg-card p-3 shadow-sm">
      <div className="mb-3 flex items-center gap-2 px-2 text-sm font-black"><SlidersHorizontal className="h-4 w-4 text-primary" aria-hidden />{labels.main}</div>
      <div className="flex gap-1.5 overflow-x-auto pb-1 no-scrollbar lg:block">
        <button type="button" onClick={() => setActive('all')} className={`mb-1 flex shrink-0 items-center justify-between gap-3 rounded-xl px-3 py-2.5 text-start text-xs font-bold transition lg:w-full ${active === 'all' ? 'bg-primary text-primary-foreground shadow-sm' : 'hover:bg-muted'}`}><span>{allLabel || labels.all}</span><ChevronDown className="hidden h-3.5 w-3.5 lg:block" aria-hidden /></button>
        {categories.map((category) => <button key={category.key} type="button" onClick={() => setActive(category.key)} className={`mb-1 flex shrink-0 items-center justify-between gap-3 rounded-xl px-3 py-2.5 text-start text-xs font-bold transition lg:w-full ${active === category.key ? 'bg-primary/10 text-primary ring-1 ring-primary/15' : 'hover:bg-muted'}`}><span className="truncate">{category.title}</span><span className="shrink-0 text-[10px] text-muted-foreground">{category.productCount ?? 0}</span></button>)}
      </div>
    </div></aside>

    <div className="min-w-0 space-y-5">
      <div className="flex items-end justify-between gap-3 rounded-3xl border border-border bg-card p-4 sm:p-5"><div><p className="text-xs font-bold text-primary">{labels.explore}</p><h2 className="mt-1 text-xl font-black tracking-tight sm:text-2xl">{selected?.title ?? labels.all}</h2><p className="mt-1 text-xs text-muted-foreground">{visible.length} {labels.items}</p></div><Link href="/search" className="shrink-0 rounded-xl border border-border bg-background px-3 py-2 text-xs font-bold transition hover:border-primary/40 hover:text-primary">{locale === 'en' ? 'Search products' : locale === 'ps' ? 'محصولات ولټوئ' : 'جستجوی محصولات'}</Link></div>
