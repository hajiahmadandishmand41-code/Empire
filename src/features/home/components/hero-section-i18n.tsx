...
export function HeroSectionI18n() {
  ...
  }, [current, next, paused, reducedMotion]);
  ...
  return <section aria-label={tc('appName')} className="w-full border-b border-border bg-muted/30"><div className="mx-auto max-w-screen-xl px-3 py-3 sm:px-6 sm:py-5"><div className="grid grid-cols-1 gap-3 lg:grid-cols-4">
    <div className="relative overflow-hidden rounded-2xl shadow-xl ring-1 ring-black/5 lg:col-span-3" onMouseEnter={() => setPaused(true)} onMouseLeave={() => setPaused(false)} onFocus={() => setPaused(true)} onBlur={() => setPaused(false)}>
      <div className={cn('relative min-h-[270px] bg-gradient-to-br transition-colors duration-500 sm:min-h-[330px]', background)}><div className="pointer-events-none absolute inset-0" style={{ background: `radial-gradient(circle at 82% 18%, ${slide.accent}45 0%, transparent 36%), radial-gradient(circle at 8% 88%, rgba(255,255,255,.12) 0%, transparent 36%)` }} aria-hidden />
        <div className="relative flex min-h-[270px] items-center justify-between px-5 py-8 sm:min-h-[330px] sm:px-10 sm:py-10"><div className="max-w-2xl min-w-0 flex-1 space-y-4"><p className="text-[11px] font-bold uppercase tracking-[0.18em] text-white/70">{slide.eyebrow}</p><div className="inline-flex items-center gap-2 rounded-full border border-white/20 bg-white/10 px-3 py-1.5 text-[11px] font-bold text-white backdrop-blur-sm"><Sparkles className="h-3.5 w-3.5 text-amber-300" aria-hidden />{slide.badge}</div><div className="min-h-[6.75rem] sm:min-h-[7.5rem] lg:min-h-[8.25rem]"><h1 className="max-w-xl text-3xl font-black leading-[1.3] tracking-tight text-white sm:text-4xl lg:text-5xl">{slide.title}{' '}<span className="text-amber-300">{slide.highlight}</span></h1></div><div className="min-h-[5.25rem] sm:min-h-[5.5rem]"><p className="max-w-xl text-sm leading-7 text-white/85 sm:text-base">{slide.subtitle}</p></div><div className="flex flex-wrap gap-2.5 pt-1"><Link href={localize(slide.primaryHref)} className="inline-flex min-h-10 items-center gap-2 rounded-xl bg-white px-5 py-2.5 text-xs font-extrabold text-slate-950 shadow-lg transition-transform hover:-translate-y-0.5"><span>{slide.primaryCta}</span><ArrowLeft className="h-3.5 w-3.5 rtl:rotate-180" aria-hidden="true" /></Link><Link href={localize(slide.secondaryHref)} className="inline-flex min-h-10 items-center rounded-xl border border-white/25 bg-white/5 px-4 py-2.5 text-xs font-semibold text-white backdrop-blur-sm transition-colors hover:bg-white/10">{slide.secondaryCta}</Link></div></div><div className="relative ms-8 hidden shrink-0 sm:block" aria-hidden><div className="relative flex h-36 w-36 items-center justify-center rounded-[2rem] border border-white/20 bg-white/10 shadow-2xl backdrop-blur-md md:h-44 md:w-44"><Icon className="h-16 w-16 text-white/90 md:h-20 md:w-20" /></div></div></div>
      ...
      {!reducedMotion && <div className="absolute inset-x-0 bottom-0 h-0.5 bg-white/15"><div className="h-full bg-white/80" style={{ width: `${progress}%` }} /></div>}
      <button type="button" onClick={previous} aria-label={tc('previous')} className="absolute start-3 top-1/2 z-10 flex h-9 w-9 -translate-y-1/2 items-center justify-center rounded-full border border-white/15 bg-black/30 text-white transition-colors hover:bg-black/45 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white"><ChevronRight className="h-4 w-4 rtl:rotate-180" aria-hidden="true" /></button><button type="button" onClick={next} aria-label={tc('next')} className="absolute end-3 top-1/2 z-10 flex h-9 w-9 -translate-y-1/2 items-center justify-center rounded-full border border-white/15 bg-black/30 text-white transition-colors hover:bg-black/45 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white"><ChevronLeft className="h-4 w-4 rtl:rotate-180" aria-hidden="true" /></button>
      <div className="absolute inset-x-0 bottom-3 flex justify-center gap-2" role="tablist" aria-label={tc('next')}>{slides.map((_, index) => <button key={index} type="button" onClick={() => setCurrent(index)} role="tab" aria-selected={index === current} aria-label={`${index + 1}`} className={cn('rounded-full transition-colors', index === current ? 'h-2 w-7 bg-white' : 'h-2 w-2 bg-white/35')} />)}</div>
    </div>
    <div className="grid grid-cols-2 gap-2.5 lg:grid-cols-1">{quickLinks.map(({ label, href, color, Icon: QuickIcon }) => <Link key={href} href={localize(href)} className="group flex min-h-[4.25rem] items-center justify-between gap-2 rounded-xl border border-border bg-card px-3.5 py-3.5 shadow-sm transition-colors hover:border-primary/40"><span className="min-w-0 text-xs font-bold leading-tight text-foreground group-hover:text-primary">{label}</span><span className={cn('flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br text-white shadow-sm', color)}><QuickIcon className="h-[18px] w-[18px]" aria-hidden="true" /></span></Link>)}</div>
  </div></div></section>;
}
