...
export function HeroSectionI18n() {
  ...
  return (
    <section aria-label={tc('appName')} className="w-full border-b border-border bg-muted/30">
      <div className="mx-auto max-w-screen-xl px-3 py-3 sm:px-6 sm:py-5">
        <div className="grid grid-cols-1 gap-3 lg:grid-cols-4">
          <div className="relative overflow-hidden rounded-2xl shadow-xl ring-1 ring-black/5 lg:col-span-3" onMouseEnter={() => setPaused(true)} onMouseLeave={() => setPaused(false)} onFocus={() => setPaused(true)} onBlur={() => setPaused(false)}>
            <div className={cn('relative min-h-[270px] bg-gradient-to-br transition-colors duration-500 sm:min-h-[330px]', background)}>
              <div className="pointer-events-none absolute inset-0" style={{ background: `radial-gradient(circle at 82% 18%, ${slide.accent}45 0%, transparent 36%), radial-gradient(circle at 8% 88%, rgba(255,255,255,.12) 0%, transparent 36%)` }} aria-hidden />
              <div className="pointer-events-none absolute -end-10 -top-10 h-52 w-52 rounded-full bg-white/10 blur-3xl" aria-hidden />
              <div className="relative flex min-h-[270px] items-center justify-between px-5 py-8 sm:min-h-[330px] sm:px-10 sm:py-10">
                <div className="max-w-2xl flex-1 space-y-4">
                  ...
                  <div className="flex flex-wrap gap-2.5 pt-1">
                    <Link href={localize(slide.primaryHref)} className="inline-flex items-center gap-2 rounded-xl bg-white px-5 py-2.5 text-xs font-extrabold text-slate-950 shadow-lg transition hover:-translate-y-0.5 hover:bg-white/95 active:scale-95">{slide.primaryCta}<ArrowLeft className="h-3.5 w-3.5 rtl:rotate-180" aria-hidden /></Link>
                    <Link href={localize(slide.secondaryHref)} className="inline-flex items-center rounded-xl border border-white/25 bg-white/5 px-4 py-2.5 text-xs font-semibold text-white backdrop-blur-sm transition hover:-translate-y-0.5 hover:bg-white/10 active:scale-95">{slide.secondaryCta}</Link>
                  </div>
                </div>
                <div className="relative ms-8 hidden shrink-0 sm:block" aria-hidden>
                  <div className="absolute inset-0 animate-pulse-ring rounded-[2rem]" style={{ background: `radial-gradient(circle, ${slide.accent}75, transparent)` }} />
            ...
            <button type="button" onClick={next} aria-label={tc('next')} className="absolute end-3 top-1/2 z-10 flex h-9 w-9 -translate-y-1/2 items-center justify-center rounded-full border border-white/15 bg-black/30 text-white backdrop-blur-sm transition hover:scale-110 hover:bg-black/50 active:scale-95"><ChevronLeft className="h-4 w-4" aria-hidden /></button>
            <div className="absolute inset-x-0 bottom-3 flex justify-center gap-2" role="tablist" aria-label={tc('next')}>
              {slides.map((_, index) => <button key={index} type="button" onClick={() => setCurrent(index)} role="tab" aria-selected={index === current} aria-label={`${index + 1}`} className={cn('rounded-full transition-all', index === current ? 'h-2 w-7 bg-white' : 'h-2 w-2 bg-white/35 hover:bg-white/60')} />)}
            </div>
          </div>
          <div className="grid grid-cols-2 gap-2.5 lg:grid-cols-1">
            {quickLinks.map(({ label, href, color, Icon: QuickIcon }) => (
              <Link key={href} href={localize(href)} className="group flex items-center justify-between gap-2 rounded-xl border border-border bg-card px-3.5 py-3.5 shadow-sm transition hover:-translate-y-0.5 hover:border-primary/40 hover:shadow-md"><span className="text-xs font-bold leading-tight text-foreground transition-colors group-hover:text-primary">{label}</span><span className={cn('flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br text-white shadow-sm transition-transform group-hover:scale-110', color)}><QuickIcon className="h-[18px] w-[18px]" aria-hidden /></span></Link>
            ))}
          </div>
        </div>
      </div>
