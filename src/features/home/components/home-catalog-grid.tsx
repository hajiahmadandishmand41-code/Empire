...
export function HomeCatalogGrid({ products, locale = 'fa', currency = 'AFN' }: { products: ProductSummary[]; locale?: string; currency?: string }) {
  ...
  return <section className="border-y border-border bg-background py-4 sm:py-8" aria-label={copy.title}>
    <div className="mx-auto max-w-screen-2xl px-2.5 sm:px-6">
      ...
      <div className="product-grid grid grid-cols-3 gap-2 sm:grid-cols-3 sm:gap-3 lg:grid-cols-4 xl:grid-cols-5">{unique.map((product) => <MarketplaceProductCard key={product.id} product={product} locale={locale} currency={currency} view="grid" />)}</div>
    </div>
  </section>;
}
