...
export async function TrustSection() {
  ...
  return (
    <section aria-label={heading} className="section-band section-band-alt">
      <Container size="xl">
        <div className="mb-4 flex items-end justify-between gap-3">
          <div>
            <p className="text-[10px] font-extrabold uppercase tracking-[0.16em] text-primary">{brand}</p>
            <h2 className="mt-1 text-base font-black tracking-tight text-foreground sm:text-xl">{heading}</h2>
          </div>
        ...
        <div className="grid grid-cols-2 gap-2.5 sm:grid-cols-5 sm:gap-3">
          {items.map(({ key, icon: Icon }) => {
            ...
            return (
              <div key={key} className="surface-card surface-card-interactive group/trust flex items-center gap-3 p-3">
                <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-primary/15 to-primary/5 text-primary transition-all duration-300 group-hover/trust:shadow-glow">
                  <Icon className="h-5 w-5" aria-hidden="true" />
                </span>
                <div className="min-w-0">
                  <p className="truncate text-xs font-extrabold text-foreground sm:text-[13px]">{label}</p>
                  <p className="mt-0.5 truncate text-[10px] text-muted-foreground sm:text-[11px]">{sub}</p>
                </div>
              </div>
            );
          })}
