...
export function HeaderSearchBar({ locale }: HeaderSearchBarProps) {
  ...
  const safeLocale = locale === 'ps' || locale === 'en' ? locale : 'fa';

  return (
    <form action={`/${safeLocale}/search`} method="get" role="search" className="w-full" aria-label={t('search')}>
      <div className="group relative w-full">
        <button type="submit" aria-label={t('search')} className="pointer-events-auto absolute start-3.5 top-1/2 z-10 rounded-md text-muted-foreground transition-colors group-focus-within:text-primary group-hover:text-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">
