...
export async function BrandsSection({ locale }: { locale: Locale }) {
  ...
  return (
    <section aria-labelledby="brands-title" className="border-b border-border bg-background py-5 sm:py-7">
      <div className="mx-auto max-w-screen-xl px-3 sm:px-6">
        <div className="mb-4 flex items-center justify-between gap-3">
          <div className="flex min-w-0 items-center gap-2.5">
            <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-primary/10 text-primary"><Tags className="h-4 w-4" aria-hidden /></span>
            <div className="min-w-0"><h2 id="brands-title" className="truncate text-sm font-black sm:text-base">{t.title}</h2><p className="line-clamp-1 text-[10px] text-muted-foreground sm:text-xs">{t.subtitle}</p></div>
          </div>
        ...
        <div className="grid grid-cols-4 gap-2.5 sm:grid-cols-6 sm:gap-4 lg:grid-cols-10">
          {result.items.map((store) => (
            <Link key={store.id} href={`/store/${store.id}` as never} aria-label={store.shopName} className="group flex min-w-0 flex-col items-center gap-1.5 rounded-2xl p-1 text-center transition-transform duration-200 hover:-translate-y-0.5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary">
              <span className="relative flex h-14 w-14 items-center justify-center overflow-hidden rounded-full border border-border bg-background shadow-sm ring-2 ring-background transition-[transform,box-shadow,border-color] group-hover:scale-105 group-hover:border-primary/30 group-hover:shadow-md sm:h-16 sm:w-16">
                {store.logoUrl ? <Image src={store.logoUrl} alt="" fill sizes="64px" className="object-cover" /> : <span className="text-sm font-black text-primary">{store.shopName.charAt(0)}</span>}
