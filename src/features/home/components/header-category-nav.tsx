...
import { Smartphone, Home as HomeIcon, Dumbbell, Baby, Zap, LayoutGrid, ShoppingBag, BookOpen, Shirt, Watch, Package, Sparkles } from 'lucide-react';
      ...
      <div className="mx-auto max-w-screen-xl px-6">
        <div className="flex items-center gap-0 overflow-x-auto py-0 no-scrollbar">
          ...
          {CATEGORY_KEYS.map((key) => {
            ...
            return (
              <Link key={key} href={`/category/${key}`} className="flex shrink-0 items-center gap-1.5 border-b-2 border-transparent px-3.5 py-3 text-xs font-medium text-muted-foreground transition-all hover:border-rose-500 hover:text-rose-600 dark:hover:border-rose-400 dark:hover:text-rose-300">
                <Icon className="h-3.5 w-3.5" aria-hidden="true" />
                <span>{tCategories(`${key}.title` as Parameters<typeof tCategories>[0])}</span>
              </Link>
            );
          })}
          <Link href="/traditional" className="flex shrink-0 items-center gap-1.5 border-b-2 border-transparent px-3.5 py-3 text-xs font-bold text-primary transition-all hover:border-primary hover:bg-primary/5">
