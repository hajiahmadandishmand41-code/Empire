  ...
  return (
    <section className="mx-auto max-w-screen-xl px-3 sm:px-6">
      <div className="flex items-center justify-between gap-4 rounded-2xl border border-violet-300/20 bg-gradient-to-r from-slate-950 via-indigo-950 to-rose-950 px-4 py-4 text-white shadow-sm sm:px-6">
        ...
        <Link href="/seller/apply" className="inline-flex shrink-0 items-center gap-2 rounded-xl bg-white px-3.5 py-2.5 text-xs font-bold text-slate-950 transition-transform hover:-translate-y-0.5"><span>{copy.cta}</span><ArrowLeft className="h-3.5 w-3.5 rtl:rotate-180" /></Link>
      </div>
    </section>
  );
}
