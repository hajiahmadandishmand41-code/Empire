...
function fallbackSlides(locale: string): VisualSlide[] {
  const fa: VisualSlide[] = [
    { id: 'local', kind: 'visual', title: 'محصولات وطنی', subtitle: 'حمایت از تولید ملی با انتخاب‌های باکیفیت و اصیل.', ctaLabel: 'مشاهده محصولات', href: '/traditional', theme: 'from-rose-100 via-fuchsia-50 to-sky-100 dark:from-rose-950 dark:via-fuchsia-950 dark:to-slate-950', icon: Sparkles },
    { id: 'home', kind: 'visual', title: 'محصولات خانه و زندگی', subtitle: 'برای خانه‌ای زیباتر و خریدی کاربردی‌تر، انتخاب‌های تازه را ببینید.', ctaLabel: 'خرید برای خانه', href: '/categories', theme: 'from-pink-100 via-white to-sky-100 dark:from-pink-950 dark:via-slate-950 dark:to-sky-950', icon: Home },
    { id: 'stores', kind: 'visual', title: 'فروشگاه‌های ایشاپ', subtitle: 'فروشگاه‌های معتبر و فروشندگان را یکجا کشف کنید.', ctaLabel: 'مشاهده فروشگاه‌ها', href: '/stores', theme: 'from-fuchsia-100 via-violet-50 to-blue-100 dark:from-fuchsia-950 dark:via-violet-950 dark:to-blue-950', icon: Store },
    { id: 'discounts', kind: 'visual', title: 'تخفیف‌های ویژه', subtitle: 'فقط پیشنهادهایی با تخفیف واقعی بیشتر از ۲۰٪ در این بخش.', ctaLabel: 'مشاهده تخفیف‌ها', href: '/discounts', theme: 'from-rose-100 via-sky-50 to-blue-100 dark:from-rose-950 dark:via-slate-950 dark:to-blue-950', icon: Tags },
    { id: 'discover', kind: 'visual', title: 'کشف بهترین‌ها', subtitle: 'پیشنهادهای هوشمند بر اساس محبوبیت، تازگی و سلیقه شما.', ctaLabel: 'شروع کشف', href: '/discover', theme: 'from-sky-100 via-pink-50 to-fuchsia-100 dark:from-sky-950 dark:via-slate-950 dark:to-fuchsia-950', icon: Compass },
  ];
...
export function HomepageHeroCarousel({ banners = [], locale = 'fa' }: { banners?: CampaignBanner[]; locale?: string }) {
  ...
  return (
    <section className="mx-auto max-w-screen-xl px-3 pt-3 sm:px-6 sm:pt-5" aria-label={labels.aria}>
      <div className="group relative overflow-hidden rounded-[26px] border border-white/70 bg-card shadow-premium dark:border-white/10" onMouseEnter={() => setPaused(true)} onMouseLeave={() => setPaused(false)} onFocus={() => setPaused(true)} onBlur={() => setPaused(false)} onPointerDown={onPointerDown} onPointerUp={onPointerUp}>
        {slide.kind === 'image' ? (
          <div className="relative h-[260px] sm:h-[340px]">
            ...
            <div className="absolute inset-0 bg-gradient-to-t from-slate-950/70 via-fuchsia-950/10 to-transparent" aria-hidden="true" />
            <div key={slide.id} className={cn('absolute inset-x-0 bottom-0 p-4 sm:p-8 lg:p-10', !reduceMotion && 'hero-copy-in')} dir={isEnglish ? 'ltr' : 'rtl'}>
              <div className="max-w-[86%] text-white sm:max-w-2xl"><span className="inline-flex items-center gap-1.5 rounded-full bg-white/15 px-3 py-1.5 text-[10px] font-extrabold backdrop-blur-sm"><Sparkles className="h-3 w-3" />ایشاپ</span>{slide.title ? <h1 className="mt-2 line-clamp-2 text-xl font-black leading-[1.15] sm:text-4xl">{slide.title}</h1> : null}{slide.subtitle ? <p className="mt-1.5 line-clamp-2 max-w-xl text-[11px] leading-4 text-white/85 sm:text-sm sm:leading-7">{slide.subtitle}</p> : null}{slide.ctaLabel && slide.href ? <Link href={slide.href as never} className="mt-3 inline-flex min-h-9 items-center gap-2 rounded-xl bg-white px-3 py-2 text-xs font-extrabold text-slate-900 shadow-sm sm:min-h-10 sm:px-4 sm:text-sm">{slide.ctaLabel}<ChevronLeft className="h-4 w-4 rtl:rotate-180" /></Link> : null}</div>
            </div>
          </div>
        ) : (
          <div className={cn('relative h-[260px] overflow-hidden bg-gradient-to-br p-4 shadow-inner dark:text-white sm:h-[340px] sm:p-8', slide.theme)} dir={isEnglish ? 'ltr' : 'rtl'}>
            <div className="absolute -end-24 -top-24 h-64 w-64 rounded-full bg-white/60 blur-3xl dark:bg-white/10" aria-hidden="true" />
            <div className="absolute -start-20 -bottom-28 h-64 w-64 rounded-full bg-white/45 blur-3xl dark:bg-white/5" aria-hidden="true" />
            <div className="absolute inset-y-0 end-0 hidden w-1/3 bg-gradient-to-l from-white/30 via-white/10 to-transparent sm:block" aria-hidden="true" />
            <div className="relative flex h-full items-center"><div className="max-w-[86%] sm:max-w-2xl">
              <div className="mb-2 inline-flex items-center gap-1.5 rounded-full border border-white/70 bg-white/60 px-3 py-1.5 text-[10px] font-extrabold text-slate-800 shadow-sm backdrop-blur-md dark:border-white/15 dark:bg-white/10 dark:text-white"><Sparkles className="h-3 w-3" />ایشاپ</div>
              <h1 className="line-clamp-2 text-xl font-black leading-[1.15] text-slate-900 drop-shadow-sm dark:text-white sm:text-4xl lg:text-5xl">{slide.title}</h1>
              <p className="mt-2 line-clamp-2 max-w-xl text-[11px] font-medium leading-4 text-slate-700 sm:text-sm sm:leading-7 dark:text-white/80">{slide.subtitle}</p>
              <Link href={slide.href as never} className="mt-3 inline-flex min-h-9 items-center gap-2 rounded-xl bg-gradient-to-r from-rose-600 via-fuchsia-600 to-sky-600 px-3 py-2 text-xs font-extrabold text-white shadow-lg shadow-fuchsia-900/15 transition-transform duration-200 hover:-translate-y-0.5 hover:shadow-xl sm:min-h-10 sm:px-4 sm:text-sm dark:from-rose-500 dark:via-fuchsia-500 dark:to-sky-500">{slide.ctaLabel}<ChevronLeft className="h-4 w-4 rtl:rotate-180" /></Link>
            </div><div className="ms-auto hidden h-36 w-36 shrink-0 items-center justify-center rounded-[32px] border border-white/70 bg-white/45 shadow-2xl backdrop-blur-md sm:flex lg:h-48 lg:w-48 dark:border-white/15 dark:bg-white/10" aria-hidden="true"><SlideIcon className="h-20 w-20 text-slate-800 drop-shadow-sm dark:text-white lg:h-24 lg:w-24" /></div></div>
          </div>
        )}
        {count > 1 ? <><button type="button" onClick={() => move(-1)} aria-label={labels.previous} className="absolute start-2 top-1/2 z-10 flex h-9 w-9 -translate-y-1/2 items-center justify-center rounded-full border border-white/40 bg-black/25 text-white shadow-sm backdrop-blur-sm hover:bg-black/40 sm:start-3"><ChevronRight className="h-4 w-4 rtl:rotate-180" /></button><button type="button" onClick={() => move(1)} aria-label={labels.next} className="absolute end-2 top-1/2 z-10 flex h-9 w-9 -translate-y-1/2 items-center justify-center rounded-full border border-white/40 bg-black/25 text-white shadow-sm backdrop-blur-sm hover:bg-black/40 sm:end-3"><ChevronLeft className="h-4 w-4 rtl:rotate-180" /></button><div className="absolute inset-x-0 bottom-2.5 z-10 flex justify-center gap-1.5 sm:bottom-3" aria-label="Slide progress">{slides.map((item, dot) => <button key={item.id} type="button" onClick={() => setIndex(dot)} aria-label={`${dot + 1}`} aria-current={index === dot} className={cn('h-1.5 rounded-full transition-all', index === dot ? 'w-7 bg-white' : 'w-1.5 bg-white/45')} />)}</div></> : null}
