...
function fallbackSlides(locale: string): VisualSlide[] {
  const fa: VisualSlide[] = [
    ...
    { id: 'discounts', kind: 'visual', title: 'تخفیف‌های ویژه', subtitle: 'قیمت‌های جذاب و پیشنهادهایی که ارزش خرید بیشتری دارند.', ctaLabel: 'مشاهده تخفیف‌ها', href: '/shop?badge=sale', theme: 'from-emerald-100 via-teal-50 to-cyan-100 dark:from-emerald-950 dark:via-slate-950 dark:to-cyan-950', icon: Tags },
    ...
    <section className="mx-auto max-w-screen-xl px-3 pt-3 sm:px-6 sm:pt-5" aria-label={labels.aria}>
      <div className="group relative overflow-hidden rounded-[24px] border border-border bg-card shadow-premium" onMouseEnter={() => setPaused(true)} onMouseLeave={() => setPaused(false)} onFocus={() => setPaused(true)} onBlur={() => setPaused(false)} onPointerDown={onPointerDown} onPointerUp={onPointerUp}>
        {slide.kind === 'image' ? (
          ...
          <div className={cn('relative h-[258px] overflow-hidden bg-gradient-to-br p-4 dark:text-white sm:h-[340px] sm:p-8', slide.theme)} dir={isEnglish ? 'ltr' : 'rtl'}>
            <div className="absolute -end-20 -top-20 h-56 w-56 rounded-full bg-white/30 blur-2xl dark:bg-white/10" aria-hidden="true" /><div className="absolute -start-16 -bottom-24 h-52 w-52 rounded-full bg-white/25 blur-2xl dark:bg-white/5" aria-hidden="true" />
            <div className="relative flex h-full items-center"><div className="max-w-[84%] sm:max-w-2xl">
              <div className="mb-2 inline-flex items-center gap-1.5 rounded-full border border-black/10 bg-white/55 px-3 py-1.5 text-[10px] font-extrabold text-slate-800 backdrop-blur-sm dark:border-white/15 dark:bg-white/10 dark:text-white"><Sparkles className="h-3 w-3" />ایشاپ</div>
              <h1 className="line-clamp-2 text-xl font-black leading-[1.15] text-slate-900 dark:text-white sm:text-4xl lg:text-5xl">{slide.title}</h1>
              <p className="mt-2 line-clamp-2 max-w-xl text-[11px] font-medium leading-4 text-slate-700 dark:text-white/80 sm:text-sm sm:leading-7">{slide.subtitle}</p>
              <Link href={slide.href as never} className="mt-3 inline-flex min-h-9 items-center gap-2 rounded-xl bg-slate-900 px-3 py-2 text-xs font-extrabold text-white shadow-md transition-transform duration-200 hover:-translate-y-0.5 dark:bg-white dark:text-slate-950 sm:min-h-10 sm:px-4 sm:text-sm">{slide.ctaLabel}<ChevronLeft className="h-4 w-4" /></Link>
            </div><div className="ms-auto hidden h-36 w-36 shrink-0 items-center justify-center rounded-[32px] border border-white/40 bg-white/35 shadow-xl backdrop-blur-sm sm:flex lg:h-48 lg:w-48 dark:border-white/15 dark:bg-white/10" aria-hidden="true"><SlideIcon className="h-20 w-20 text-slate-800 dark:text-white lg:h-24 lg:w-24" /></div></div>
          </div>
        )}
        {count > 1 ? <><button type="button" onClick={() => move(-1)} aria-label={labels.previous} className="absolute start-2 top-1/2 z-10 flex h-9 w-9 -translate-y-1/2 items-center justify-center rounded-full border border-white/30 bg-black/25 text-white backdrop-blur-sm hover:bg-black/40 sm:start-3"><ChevronRight className="h-4 w-4 rtl:rotate-180" /></button><button type="button" onClick={() => move(1)} aria-label={labels.next} className="absolute end-2 top-1/2 z-10 flex h-9 w-9 -translate-y-1/2 items-center justify-center rounded-full border border-white/30 bg-black/25 text-white backdrop-blur-sm hover:bg-black/40 sm:end-3"><ChevronLeft className="h-4 w-4 rtl:rotate-180" /></button><div className="absolute inset-x-0 bottom-2.5 z-10 flex justify-center gap-1.5 sm:bottom-3" aria-label="Slide progress">{slides.map((item, dot) => <button key={item.id} type="button" onClick={() => setIndex(dot)} aria-label={`${dot + 1}`} aria-current={index === dot} className={cn('h-1.5 rounded-full transition-all', index === dot ? 'w-7 bg-white' : 'w-1.5 bg-white/50')} />)}</div></> : null}
