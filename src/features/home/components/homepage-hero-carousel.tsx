...
function bannerCopy(locale: string) {
  ...
  return { aria: 'کمپین‌های ویژه', next: 'کمپین قبلی', previous: 'کمپین بعدی', swipe: 'برای دیدن بکشید' };
}
      ...
      <div className="group relative isolate overflow-hidden rounded-[24px] border border-black/10 bg-card shadow-premium dark:border-white/10" onMouseEnter={() => setPaused(true)} onMouseLeave={() => setPaused(false)} onFocus={() => setPaused(true)} onBlur={() => setPaused(false)} onPointerDown={onPointerDown} onPointerUp={onPointerUp} style={{ touchAction: 'pan-y' }}>
        <div className="relative h-[250px] sm:h-[330px] lg:h-[350px]" aria-live="polite">
          {slide.kind === 'image' ? (
            ...
            <div className={cn('relative h-full overflow-hidden bg-gradient-to-br p-4 text-white shadow-inner sm:p-8', slide.theme)} dir={isEnglish ? 'ltr' : 'rtl'}><div className="absolute -end-24 -top-24 h-64 w-64 rounded-full bg-white/10 blur-3xl" aria-hidden="true" /><div className="absolute -start-20 -bottom-28 h-64 w-64 rounded-full bg-rose-300/10 blur-3xl" aria-hidden="true" /><div className="relative flex h-full items-center"><div className="max-w-[88%] sm:max-w-2xl"><div className="mb-2 inline-flex items-center gap-1.5 rounded-full border border-white/15 bg-white/10 px-3 py-1.5 text-[10px] font-extrabold text-white shadow-sm backdrop-blur-md"><Sparkles className="h-3 w-3" aria-hidden="true" />ایشاپ</div><h1 className="line-clamp-2 text-xl font-black leading-[1.15] text-white drop-shadow-sm sm:text-4xl lg:text-5xl">{slide.title}</h1><p className="mt-2 line-clamp-2 max-w-xl text-[11px] font-medium leading-4 text-white/78 sm:text-sm sm:leading-7">{slide.subtitle}</p><Link href={slide.href as never} className="mt-3 inline-flex min-h-10 items-center gap-2 rounded-xl bg-white px-3.5 py-2 text-xs font-extrabold text-slate-900 shadow-lg transition hover:-translate-y-0.5 sm:px-4 sm:text-sm">{slide.ctaLabel}<ChevronLeft className="h-4 w-4 rtl:rotate-180" aria-hidden="true" /></Link></div><div className="ms-auto hidden h-36 w-36 shrink-0 items-center justify-center rounded-[30px] border border-white/15 bg-white/10 shadow-xl backdrop-blur-md sm:flex lg:h-48 lg:w-48"><SlideIcon className="h-20 w-20 text-white/90 lg:h-24 lg:w-24" aria-hidden="true" /></div></div></div>
          )}
        </div>
