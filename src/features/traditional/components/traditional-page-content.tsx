...
const CATEGORY_EMOJI: Record<(typeof CATEGORY_KEYS)[number], string> = {
  all: '🇦🇫', carpet: '🎨', saffron: '🌸', driedFruits: '🍇', handicrafts: '🏺', localClothing: '👘', honey: '🍯', nuts: '🥜', gemstones: '💎', traditional: '✨',
};
...
async function fetchProducts(categoryKey: string, page: number): Promise<Product[]> {
  try {
    const params = new URLSearchParams({ limit: String(PAGE_SIZE), offset: String(page * PAGE_SIZE), isTraditional: 'true', ...(categoryKey !== 'all' ? { categoryKey } : {}) });
...
function TraditionalProductCard({ product, locale, currency, labels }: { product: Product; locale: string; currency: string; labels: { discount: string; video: string; removeWishlist: string; addWishlist: string; buy: string } }) {
  ...
  return <article className="group relative flex min-w-0 flex-col overflow-hidden rounded-lg border border-border bg-card shadow-sm transition-[border-color,box-shadow,transform] duration-200 hover:-translate-y-px hover:border-primary/25 hover:shadow-md">
    <Link href={`/${locale}/shop/${product.slug}`} className="relative block aspect-square overflow-hidden bg-muted/40">
      ...
      <button type="button" onClick={(event) => { event.preventDefault(); event.stopPropagation(); toggle(product.slug); }} className="absolute end-2 top-2 flex h-9 w-9 items-center justify-center rounded-full border border-border/70 bg-background/90 shadow-sm backdrop-blur-sm transition-colors hover:bg-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring" aria-label={isWishlisted ? labels.removeWishlist : labels.addWishlist}><Heart className={cn('h-4 w-4 transition-colors', isWishlisted ? 'fill-destructive text-destructive' : 'text-muted-foreground')} aria-hidden="true" /></button>
    </Link>
    <div className="flex min-h-[9.5rem] flex-col gap-1.5 p-3">{product.category && <span className="truncate text-[10px] font-medium uppercase tracking-wide text-primary">{product.category.name}</span>}<Link href={`/${locale}/shop/${product.slug}`} className="line-clamp-2 min-h-[2.5rem] text-sm font-semibold leading-5 text-foreground transition-colors hover:text-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">{product.name}</Link><div className="min-h-4">{product.rating !== undefined && <div className="flex items-center gap-1" aria-label={product.rating.toFixed(1)}><Star className="h-3 w-3 fill-amber-400 text-amber-400" aria-hidden="true" /><span className="text-xs text-muted-foreground">{product.rating.toFixed(1)}</span></div>}</div><div className="mt-auto flex items-center justify-between gap-2 border-t border-border pt-2.5"><div className="min-w-0">{product.comparePrice && product.comparePrice > product.price && <span className="block truncate text-[10px] text-muted-foreground line-through">{formatPrice(product.comparePrice, currency, locale)}</span>}<span className="block truncate text-sm font-extrabold text-foreground">{formatPrice(product.price, currency, locale)}</span></div><Link href={`/${locale}/shop/${product.slug}`} className="inline-flex min-h-9 shrink-0 items-center gap-1 rounded-md bg-primary px-3 py-1.5 text-xs font-bold text-primary-foreground transition-colors hover:bg-primary/90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"><ShoppingCart className="h-3 w-3" aria-hidden="true" />{labels.buy}</Link></div></div>
  </article>;
}
...
export function TraditionalPageContent({ locale = 'fa' }: { locale?: string }) {
  ...
  return <div className="min-h-dvh">
    <section className="relative overflow-hidden bg-gradient-to-br from-emerald-900 via-teal-800 to-emerald-900 py-10 sm:py-14" aria-labelledby="traditional-title"><div className="pointer-events-none absolute inset-0 opacity-20" aria-hidden="true"><div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top_left,_var(--tw-gradient-stops))] from-amber-400/30 via-transparent to-transparent" /><div className="absolute inset-0 bg-[radial-gradient(ellipse_at_bottom_right,_var(--tw-gradient-stops))] from-teal-400/20 via-transparent to-transparent" /></div><div className="relative mx-auto max-w-7xl px-4 text-center"><div className="mb-3 inline-flex items-center gap-2 rounded-full bg-white/10 px-4 py-1.5 text-xs font-semibold text-emerald-200 ring-1 ring-white/20 backdrop-blur-sm">{t('badge')}</div><h1 id="traditional-title" className="font-display text-3xl font-extrabold leading-tight text-white sm:text-4xl lg:text-5xl">{t('title')}</h1><p className="mx-auto mt-3 max-w-2xl text-sm leading-6 text-emerald-200 sm:text-base">{t('subtitle')}</p></div></section>
    <div className="border-b border-border bg-background/95 backdrop-blur-md"><div className="mx-auto max-w-7xl px-4"><div className="no-scrollbar flex gap-2 overflow-x-auto py-3" role="tablist" aria-label={t('title')}>{CATEGORY_KEYS.map((key) => <button key={key} type="button" role="tab" aria-selected={activeCategory === key} onClick={() => setActiveCategory(key)} className={cn('inline-flex min-h-10 shrink-0 items-center gap-1.5 rounded-md px-3.5 py-2 text-xs font-bold transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring', activeCategory === key ? 'bg-primary text-primary-foreground shadow-sm' : 'bg-muted text-muted-foreground hover:bg-accent hover:text-accent-foreground')}><span aria-hidden="true">{CATEGORY_EMOJI[key]}</span>{t(`categories.${key}`)}</button>)}</div></div></div>
    <div className="mx-auto max-w-7xl px-4 py-6 sm:py-8">
      {!loading && products.length === 0 && <div className="flex flex-col items-center gap-3 py-16 text-center"><div className="flex h-20 w-20 items-center justify-center rounded-2xl bg-muted/60" aria-hidden="true"><Package className="h-10 w-10 text-muted-foreground/40" /></div><p className="text-sm font-medium text-foreground">{t('emptyTitle')}</p><p className="max-w-sm text-xs leading-5 text-muted-foreground">{t('emptyDescription')}</p><button type="button" onClick={() => setActiveCategory('all')} className="mt-2 inline-flex min-h-10 items-center rounded-md bg-primary px-5 py-2.5 text-xs font-bold text-primary-foreground transition-colors hover:bg-primary/90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">{t('showAll')}</button></div>}
      {products.length > 0 && <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 sm:gap-4 md:grid-cols-4 lg:grid-cols-5">{products.map((product) => <TraditionalProductCard key={product.id} product={product} locale={locale} currency="AFN" labels={labels} />)}</div>}
