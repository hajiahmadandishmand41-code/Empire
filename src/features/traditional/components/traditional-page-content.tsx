...
import { Package, ShoppingCart, Star, Loader2, Heart, Search, SlidersHorizontal, Sparkles, ChevronDown } from 'lucide-react';
...
function ProductSkeleton() { return <div className="overflow-hidden rounded-2xl border border-border bg-card"><div className="aspect-square animate-pulse bg-muted" /><div className="space-y-2.5 p-3"><div className="h-2.5 w-16 animate-pulse rounded bg-muted" /><div className="h-3.5 w-full animate-pulse rounded bg-muted" /><div className="h-3 w-3/4 animate-pulse rounded bg-muted" /><div className="mt-2 flex items-center justify-between border-t border-border pt-2.5"><div className="h-4 w-16 animate-pulse rounded bg-muted" /><div className="h-9 w-20 animate-pulse rounded-md bg-muted" /></div></div></div>; }

function ProductCard({ product, locale, currency, labels }: { product: Product; locale: string; currency: string; labels: { discount: string; video: string; removeWishlist: string; addWishlist: string; buy: string } }) {
  ...
  return <article className="group relative flex min-w-0 flex-col overflow-hidden rounded-2xl border border-border bg-card shadow-sm transition-all duration-200 hover:-translate-y-1 hover:border-primary/30 hover:shadow-lg">
    <Link href={`/shop/${product.slug}` as never} className="relative block aspect-square overflow-hidden bg-muted/40">
      {imageUrl ? <Image src={imageUrl} alt={product.name} fill className="object-cover transition-transform duration-500 group-hover:scale-105" sizes="(max-width: 640px) 50vw, (max-width: 768px) 33vw, 25vw" /> : <div className="flex h-full w-full items-center justify-center bg-muted"><Package className="h-10 w-10 text-muted-foreground/40" aria-hidden="true" /></div>}
    ...
    <div className="flex min-h-[9.5rem] flex-col gap-1.5 p-3"><Link href={`/shop/${product.slug}` as never} className="line-clamp-2 min-h-[2.5rem] text-sm font-semibold leading-5 text-foreground transition-colors hover:text-primary">{product.name}</Link>{product.category && <span className="truncate text-[10px] font-semibold uppercase tracking-wide text-primary">{product.category.name}</span>}<div className="min-h-4">{product.rating !== undefined && <div className="flex items-center gap-1"><Star className="h-3 w-3 fill-amber-400 text-amber-400" aria-hidden="true" /><span className="text-xs text-muted-foreground">{product.rating.toFixed(1)}</span></div>}</div><div className="mt-auto flex items-center justify-between gap-2 border-t border-border pt-2.5"><div className="min-w-0">{product.comparePrice && product.comparePrice > product.price && <span className="block truncate text-[10px] text-muted-foreground line-through">{formatPrice(product.comparePrice, currency, locale)}</span>}<span className="block truncate text-sm font-extrabold text-foreground">{formatPrice(product.price, currency, locale)}</span></div><Link href={`/shop/${product.slug}` as never} className="inline-flex min-h-9 shrink-0 items-center gap-1 rounded-md bg-primary px-3 py-1.5 text-xs font-bold text-primary-foreground hover:bg-primary/90"><ShoppingCart className="h-3 w-3" aria-hidden="true" />{labels.buy}</Link></div></div>
  </article>;
}
...
export function TraditionalPageContent({ locale = 'fa' }: { locale?: string }) {
  ...
  const loadProducts = useCallback(async (categoryKey: string, pageIndex: number, reset = false) => { setLoading(true); try { const newProducts = await fetchProducts(categoryKey, pageIndex, appliedSearch, sort, minPrice, maxPrice); setProducts((previous) => reset ? newProducts : [...previous, ...newProducts]); setHasMore(newProducts.length === PAGE_SIZE); } finally { setLoading(false); } }, [appliedSearch, sort, minPrice, maxPrice]);
  useEffect(() => { setPage(0); setProducts([]); setHasMore(true); void loadProducts(activeCategory, 0, true); }, [activeCategory, appliedSearch, sort, minPrice, maxPrice, loadProducts]);
  useEffect(() => { const element = loaderRef.current; if (!element) return; const observer = new IntersectionObserver((entries) => { if (entries[0]?.isIntersecting && !loading && hasMore && products.length > 0) { const nextPage = page + 1; setPage(nextPage); void loadProducts(activeCategory, nextPage); } }, { rootMargin: '300px' }); observer.observe(element); return () => observer.disconnect(); }, [loading, hasMore, products.length, page, activeCategory, loadProducts]);
  const labels = { discount: t('discount'), video: t('video'), removeWishlist: t('removeWishlist'), addWishlist: t('addWishlist'), buy: t('buy') };
  const submitSearch = (event: React.FormEvent) => { event.preventDefault(); setAppliedSearch(search); };
  ...
  return <div className="min-h-dvh">
    <section className="relative isolate overflow-hidden bg-gradient-to-br from-emerald-950 via-teal-900 to-emerald-800 py-12 sm:py-16">
      <div className="pointer-events-none absolute -start-20 -top-24 h-72 w-72 rounded-full bg-amber-400/10 blur-3xl" /><div className="pointer-events-none absolute -end-24 bottom-0 h-80 w-80 rounded-full bg-emerald-300/10 blur-3xl" />
      <div className="relative mx-auto grid max-w-7xl items-center gap-8 px-4 lg:grid-cols-[1fr_auto]">
        <div className="max-w-3xl"><div className="mb-4 inline-flex items-center gap-2 rounded-full bg-white/10 px-4 py-1.5 text-xs font-semibold text-emerald-100 ring-1 ring-white/20 backdrop-blur">{t('badge')}</div><h1 className="text-3xl font-black tracking-tight text-white sm:text-5xl">{t('title')}</h1><p className="mt-4 max-w-2xl text-sm leading-7 text-emerald-100/85 sm:text-base">{t('subtitle')}</p><div className="mt-6 flex flex-wrap gap-2 text-xs font-semibold text-white/90"><span className="rounded-full bg-white/10 px-3 py-1.5">{t('heroPill1')}</span><span className="rounded-full bg-white/10 px-3 py-1.5">{t('heroPill2')}</span><span className="rounded-full bg-white/10 px-3 py-1.5">{t('heroPill3')}</span></div></div>
    ...
    <div className="sticky top-0 z-20 border-b border-border bg-background/95 shadow-sm backdrop-blur-md"><div className="mx-auto max-w-7xl px-4"><div className="no-scrollbar flex gap-2 overflow-x-auto py-3" role="tablist" aria-label={t('title')}>{CATEGORY_KEYS.map((key) => <button key={key} type="button" role="tab" aria-selected={activeCategory === key} onClick={() => setActiveCategory(key)} className={cn('inline-flex min-h-10 shrink-0 items-center gap-1.5 rounded-full px-4 py-2 text-xs font-bold transition-all', activeCategory === key ? 'bg-primary text-primary-foreground shadow-sm' : 'bg-muted text-muted-foreground hover:bg-accent hover:text-accent-foreground')}><span aria-hidden="true">{CATEGORY_EMOJI[key]}</span>{t(`categories.${key}`)}</button>)}</div></div></div>

    <div className="mx-auto max-w-7xl px-4 py-6 sm:py-8">
      <div className="mb-6 rounded-2xl border border-border bg-card p-4 shadow-sm sm:p-5">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between"><form onSubmit={submitSearch} className="flex flex-1 gap-2"><div className="relative flex-1"><Search className="absolute start-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" aria-hidden="true" /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder={t('searchPlaceholder')} aria-label={t('searchPlaceholder')} className="h-11 w-full rounded-xl border border-border bg-background ps-10 pe-3 text-sm outline-none ring-primary/20 transition focus:ring-2" /></div><button type="submit" className="h-11 rounded-xl bg-primary px-5 text-sm font-bold text-primary-foreground hover:bg-primary/90">{t('search')}</button></form><button type="button" onClick={() => setFiltersOpen((value) => !value)} className="inline-flex h-11 items-center justify-center gap-2 rounded-xl border border-border px-4 text-sm font-semibold hover:bg-muted"><SlidersHorizontal className="h-4 w-4" aria-hidden="true" />{t('filters')}<ChevronDown className={cn('h-4 w-4 transition-transform', filtersOpen && 'rotate-180')} /></button></div>
      ...
      {products.length > 0 && <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 sm:gap-4 md:grid-cols-4 lg:grid-cols-5">{products.map((product) => <ProductCard key={product.id} product={product} locale={locale} currency="AFN" labels={labels} />)}</div>}
