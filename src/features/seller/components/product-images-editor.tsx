...
import { PRODUCT_MAX_IMAGES } from '@/features/products/product-contract';
...
export function ProductImagesEditor({ productId, initial, initialPrimaryIndex = 0 }: Props) {
  ...
  const [compressionNote, setCompressionNote] = React.useState<string>('');
  ...
  async function onFiles(files: FileList | null) {
    ...
    setBusy(true); setProgress(0); setCompressionNote('');
    try {
      const added: string[] = [];
      let savedBytes = 0;
      for (let index = 0; index < list.length; index += 1) {
        ...
        try {
          ...
          savedBytes += Math.max(0, optimized.originalBytes - optimized.optimizedBytes);
          setCompressionNote(`${index + 1}/${list.length} · ${formatBytes(optimized.optimizedBytes)} آماده آپلود`);
          const uploaded = await uploadImageWithProgress(`/api/seller/products/${productId}/images`, optimized.file, (part) => {
            setProgress(Math.round(((index + part / 100) / list.length) * 100));
          });
          added.push(uploaded.url);
        } catch (error) {
          toast.error(`${file.name}: ${error instanceof Error ? error.message : 'آپلود ناموفق.'}`);
        }
      }
      if (added.length) {
        setImages((prev) => [...prev, ...added]);
        toast.success(`${added.length} تصویر افزوده شد${savedBytes > 0 ? `؛ ${formatBytes(savedBytes)} حجم کمتر شد.` : '.'}`);
      }
    } finally { setBusy(false); setProgress(0); setCompressionNote(''); if (inputRef.current) inputRef.current.value = ''; }
  }
  ...
  return <div className="space-y-4">
    <div className="flex flex-wrap items-center gap-3"><input ref={inputRef} type="file" accept="image/*" multiple className="hidden" onChange={(e) => { void onFiles(e.target.files); }} /><Button type="button" variant="outline" size="sm" onClick={() => inputRef.current?.click()} disabled={busy || images.length >= PRODUCT_MAX_IMAGES}><Upload className="h-4 w-4" />{busy ? `در حال آماده‌سازی… ${progress}%` : 'افزودن تصویر'}</Button><span className="text-xs text-muted-foreground">تصویر قبل از ارسال در مرورگر کوچک می‌شود تا مصرف اینترنت کم شود.</span></div>
    {busy && <div className="space-y-1"><div className="h-1.5 overflow-hidden rounded-full bg-muted"><div className="h-full rounded-full bg-primary transition-all" style={{ width: `${progress}%` }} /></div>{compressionNote && <p className="text-[11px] text-muted-foreground">{compressionNote}</p>}</div>}
