...
export function ProductImagesEditor({ productId, initial, initialPrimaryIndex = 0 }: Props) {
  ...
  async function savePatch(payload: { images?: string[]; primaryImageIndex?: number }) {
    ...
    if (!res.ok || !body?.ok) throw new Error(body?.error?.message ?? 'product_update_failed');
    return body;
  }
  ...
  async function onRemove(url: string, idx: number) {
    ...
    try {
      ...
      const body = await res.json().catch(() => null);
      if (!res.ok || !body?.ok) throw new Error(body?.error?.message ?? 'image_delete_failed');
    ...
    } catch (error) { toast.error(error instanceof Error ? error.message : 'حذف ناموفق'); }
  ...
  return <div className="space-y-4">
    <div className="flex flex-wrap items-center gap-3"><input ref={inputRef} type="file" accept={PRODUCT_IMAGE_MIME_TYPES.join(',')} multiple className="hidden" onChange={(e) => { void onFiles(e.target.files); }} /><Button type="button" variant="outline" size="sm" onClick={() => inputRef.current?.click()} disabled={busy || images.length >= PRODUCT_MAX_IMAGES}><Upload className="h-4 w-4" />{busy ? 'در حال پردازش…' : 'افزودن تصویر'}</Button><span className="text-xs text-muted-foreground">حداکثر ۱۰ مگابایت — JPEG/PNG/WebP/GIF — {images.length}/{PRODUCT_MAX_IMAGES} تصویر</span></div>
    {images.length < MIN_IMAGES && <div className="flex items-center gap-2 rounded-md border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-700 dark:border-amber-800 dark:bg-amber-950/30 dark:text-amber-400"><span className="shrink-0 text-amber-500">⚠</span>برای حرفه‌ای‌تر نمایش داده شدن محصول، حداقل ۳ تصویر پیشنهاد می‌شود. ({MIN_IMAGES - images.length} تصویر دیگر)</div>}
