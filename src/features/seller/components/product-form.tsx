...
import { X, Plus, Package, Tag, ImagePlus, Upload, Store } from 'lucide-react';
...
interface SellerBrand { id: string; name: string; slug?: string; logoUrl?: string | null }
export interface ProductFormInitial {
  id?: string; slug?: string; name?: string; shortDescription?: string; description?: string | null;
  price?: number; compareAtPrice?: number | null; categoryId?: string; brandId?: string | null; inStock?: boolean; isActive?: boolean;
...
function fileToDataUrl(file: File): Promise<string> { return new Promise((resolve, reject) => { const reader = new FileReader(); reader.onload = () => resolve(String(reader.result ?? '')); reader.onerror = () => reject(reader.error ?? new Error('file_read_failed')); reader.readAsDataURL(file); }); }
...
export function ProductForm({ mode, categories, initial, backHref }: ProductFormProps) {
  ...
  const [brand, setBrand] = React.useState<SellerBrand | null>(null); const [brandLoading, setBrandLoading] = React.useState(true);
  const [pendingImages, setPendingImages] = React.useState<PendingImage[]>([]); const [busy, setBusy] = React.useState(false); const [slugTouched, setSlugTouched] = React.useState(Boolean(initial?.slug));
  const fileInputRef = React.useRef<HTMLInputElement>(null);

  React.useEffect(() => { if (!slugTouched && name) setSlug(slugify(name)); }, [name, slugTouched]);
  React.useEffect(() => {
    let active = true;
    void fetch('/api/seller/brand', { credentials: 'include', cache: 'no-store' }).then((res) => res.json()).then((body) => { if (active && body?.ok && body.data?.id) setBrand({ id: String(body.data.id), name: String(body.data.name ?? 'برند فروشگاه'), slug: body.data.slug, logoUrl: body.data.logoUrl }); }).catch(() => { if (active) toast.error('بارگذاری برند فروشگاه ناموفق بود.'); }).finally(() => { if (active) setBrandLoading(false); });
    return () => { active = false; };
  }, []);
  ...
  function chooseImages(files: FileList | null) {
    ...
    const room = PRODUCT_MAX_IMAGES - pendingImages.length - (initial?.images?.length ?? 0);
    if (room <= 0) return toast.error(`حداکثر ${PRODUCT_MAX_IMAGES} تصویر مجاز است.`);
    const selected = Array.from(files).slice(0, room);
    const accepted = selected.flatMap((file) => {
      if (!file.type.startsWith('image/')) { toast.error(`${file.name}: فقط فایل تصویری مجاز است.`); return []; }
  ...
  async function uploadPending(productId: string) {
    ...
    for (const item of pendingImages) {
      ...
      const res = await fetch(`/api/seller/products/${productId}/images`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, credentials: 'include', body: JSON.stringify({ dataUrl }) });
      const body = await res.json().catch(() => null);
      if (!res.ok || !body?.ok) throw new Error(body?.error?.message ?? `آپلود تصویر «${item.file.name}» ناموفق بود.`);
    }
  }

  async function onSubmit(event: React.FormEvent) {
    event.preventDefault();
    if (!brand?.id) return toast.error('برند فروشگاه هنوز بارگذاری نشده است.');
    ...
    setBusy(true);
    try {
      const payload = { slug: slug.trim() || null, name: name.trim(), shortDescription: shortDescription.trim(), description: description.trim() || null, price: numericPrice, compareAtPrice: oldPrice, categoryId, brandId: brand.id, inStock: quantity > 0, isActive, stockQuantity: quantity, whatsappNumber: whatsappNumber ? cleanWhatsapp(whatsappNumber) : null, isTraditional, weightKg: weightKg.trim() ? numberValue(weightKg) : null, dimensionsJson: dimLength || dimWidth || dimHeight ? JSON.stringify({ length: numberValue(dimLength), width: numberValue(dimWidth), height: numberValue(dimHeight) }) : null, tagsJson: JSON.stringify(tags.split(/[،,]+/).map((item) => item.trim()).filter(Boolean)), attributesJson: JSON.stringify(attributes.filter((item) => item.key.trim() && item.value.trim())), ...(isEdit ? {} : { currency: 'AFN' }) };
      ...
      if (productId) await uploadPending(productId);
      toast.success(isEdit ? 'محصول و تصاویر با موفقیت به‌روزرسانی شد.' : 'محصول و تصاویر با موفقیت ثبت شد.');
  ...
  return <form onSubmit={onSubmit} className="mx-auto max-w-5xl space-y-5" dir="rtl">
    <header className="rounded-3xl border border-border bg-gradient-to-br from-primary/10 via-card to-card p-5 shadow-sm sm:p-7"><div className="flex items-center gap-3"><span className="flex h-12 w-12 items-center justify-center rounded-2xl bg-primary text-primary-foreground"><Package className="h-6 w-6" /></span><div><h1 className="text-2xl font-black">{isEdit ? 'ویرایش محصول' : 'ثبت محصول جدید'}</h1><p className="mt-1 text-xs leading-6 text-muted-foreground">اطلاعات فروشنده، فروشگاه و برند از حساب شما گرفته می‌شود و نیازی به ورود دوباره نیست.</p></div></div></header>
    <Section title="هویت محصول" icon={<Store className="h-5 w-5" />}><div className="grid gap-4 sm:grid-cols-2"><div className="rounded-2xl border border-primary/15 bg-primary/5 p-4"><div className="flex items-center gap-3">{brand?.logoUrl ? <img src={brand.logoUrl} alt="" className="h-12 w-12 rounded-xl object-cover" /> : <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-primary/10 font-black text-primary">{brand?.name?.charAt(0) ?? 'ب'}</div>}<div><p className="text-xs text-muted-foreground">برند فروشگاه</p><p className="mt-1 font-black">{brandLoading ? 'در حال بارگذاری…' : brand?.name ?? 'برند یافت نشد'}</p><p className="mt-1 text-[10px] text-muted-foreground">این محصول داخل همین برند قرار می‌گیرد.</p></div></div></div><Field label="نام محصول" required value={name} onChange={setName} placeholder="مثلاً قالی دست‌باف هراتی" /><Field label="شناسه صفحه" value={slug} onChange={(value) => { setSlugTouched(true); setSlug(value); }} placeholder="خودکار ساخته می‌شود" dir="ltr" /><div className="sm:col-span-2"><Field label="توضیح کوتاه" required value={shortDescription} onChange={setShortDescription} /><label className="mt-4 block"><span className="mb-2 block text-xs font-bold">توضیحات کامل</span><textarea value={description} onChange={(e) => setDescription(e.target.value)} rows={6} className="w-full rounded-2xl border border-input bg-background p-3.5 text-sm outline-none focus:border-primary focus:ring-2 focus:ring-primary/15" /></label></div></div></Section>
    ...
    <Section title="تصاویر محصول" icon={<ImagePlus className="h-5 w-5" />}>
      <div className="flex flex-wrap items-center gap-2"><input ref={fileInputRef} type="file" accept="image/*" multiple className="hidden" onChange={(e) => chooseImages(e.target.files)} /><Button type="button" variant="outline" onClick={() => fileInputRef.current?.click()} disabled={busy || pendingImages.length + (initial?.images?.length ?? 0) >= PRODUCT_MAX_IMAGES}><Upload className="h-4 w-4" />انتخاب تصاویر</Button><span className="text-xs text-muted-foreground">تا {PRODUCT_MAX_IMAGES} تصویر، هر تصویر حداکثر ۱۰ مگابایت. نام فایل در اطلاعات محصول نمایش داده نمی‌شود.</span></div>
      {pendingImages.length > 0 ? <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-4 lg:grid-cols-6">{pendingImages.map((item, index) => <div key={item.id} className="group relative overflow-hidden rounded-2xl border-2 border-primary/30 bg-muted"><img src={item.preview} alt={`پیش‌نمایش ${index + 1}`} className="aspect-square w-full object-cover" /><button type="button" onClick={() => removePending(item.id)} disabled={busy} className="absolute end-2 top-2 flex h-8 w-8 items-center justify-center rounded-full bg-black/65 text-white hover:bg-red-600" aria-label="حذف تصویر"><X className="h-4 w-4" /></button><span className="absolute bottom-2 start-2 rounded-full bg-black/65 px-2 py-1 text-[10px] font-bold text-white">تصویر {index + 1}</span></div>)}</div> : null}
      {isEdit && initial?.id ? <div className="mt-5"><ProductImagesEditor productId={initial.id} initial={initial.images ?? []} initialPrimaryIndex={initial.primaryImageIndex ?? 0} /></div> : <p className="mt-4 rounded-2xl border border-dashed border-border bg-muted/20 p-4 text-xs leading-6 text-muted-foreground">تصاویر همین‌جا انتخاب و پیش‌نمایش می‌شوند و هنگام ذخیره، خودکار به گالری امن محصول فرستاده می‌شوند.</p>}
    </Section>
    ...
    <div className="sticky bottom-3 z-10 flex justify-end gap-2 rounded-3xl border border-border bg-card/95 p-3 shadow-xl backdrop-blur"><Button type="button" variant="outline" onClick={() => router.push(backHref)} disabled={busy}>انصراف</Button><Button type="submit" disabled={busy || brandLoading || !brand?.id}>{busy ? 'در حال ذخیره و آپلود…' : isEdit ? 'ذخیره تغییرات' : 'ثبت محصول'}</Button></div>
  </form>;
}
