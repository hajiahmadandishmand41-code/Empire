...
function SidebarContent({
  ...
  return (
    <div className="flex h-full flex-col bg-white dark:bg-gray-950 border-e border-gray-200 dark:border-gray-800">
      ...
      <div className="flex items-center gap-3 px-4 py-4 border-b border-gray-100 dark:border-gray-800/80">
        <Link href={`/${locale}`} onClick={onNavigate} className="flex items-center gap-2.5 group">
          ...
          <div className="flex flex-col">
            ...
            <p className="text-[10px] text-emerald-500 dark:text-emerald-400 mt-0.5 font-bold tracking-wide uppercase">پنل فروشنده</p>
          </div>
        </Link>
      </div>
      ...
      {/* ── Nav ── */}
      <nav
        className="flex-1 overflow-y-auto px-3 py-3 space-y-0.5 panel-scrollbar"
...
export function SellerShell({ locale, userName, storeName, children }: SellerShellProps) {
  ...
  const items: NavItem[] = [
    ...
    { href: `${base}/wallet`, label: 'کیف پول', icon: Wallet },
        ...
        <header className="sticky top-0 z-30 flex h-14 items-center gap-3 border-b border-gray-200 dark:border-gray-800 bg-white/95 dark:bg-gray-950/95 px-4 shadow-sm backdrop-blur-md">
          ...
          <div className="flex items-center gap-2 text-sm min-w-0">
            <div className="flex items-center gap-1.5">
              <Briefcase className="h-4 w-4 text-emerald-500 shrink-0" aria-hidden />
              <span className="font-bold text-gray-900 dark:text-gray-100 hidden sm:block">پنل فروشنده</span>
            </div>
            {currentItem && (
              <>
          ...
          <div className="flex items-center gap-1.5">
            ...
            <Link
              ...
              className="hidden sm:flex items-center gap-1.5 rounded-xl bg-gradient-to-br from-emerald-500 to-teal-600 px-3.5 py-1.5 text-xs font-bold text-white hover:from-emerald-600 hover:to-teal-700 transition-all shadow-sm hover:shadow-emerald-500/25"
            >
              <Plus className="h-3.5 w-3.5" aria-hidden />
              محصول جدید
            </Link>
            ...
            <button
              className="flex h-8 w-8 items-center justify-center rounded-xl border border-gray-200 dark:border-gray-700 text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800 hover:text-emerald-600 transition-colors"
              aria-label="اعلان‌ها"
            >
