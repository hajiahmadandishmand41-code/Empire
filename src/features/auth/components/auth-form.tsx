...
export function AuthForm({ mode }: { mode: Mode }) {
  ...
  return <form onSubmit={onSubmit} className="space-y-4" noValidate aria-label={mode === 'login' ? t('login') : t('register')}>
    ...
    <p className="text-center text-sm text-muted-foreground">{mode === 'register' ? t('haveAccount') : t('noAccount')} <Link href="/auth/${mode === 'register' ? 'login' : 'register'}" className="font-bold text-primary hover:underline">{mode === 'register' ? t('signIn') : t('signUp')}</Link></p>
  </form>;
}
