...
import {
  ...
  PanelLeftClose, PanelLeftOpen, Command, ReceiptText,
} from 'lucide-react';
import { cn } from '@/lib/utils';
import { ThemeToggle } from '@/components/ui/theme-toggle';

type NavGroupId = 'overview' | 'catalog' | 'commerce' | 'marketing' | 'customers' | 'insights' | 'system';
  ...
  return (
    <Link
      ...
      className={cn(
        ...
        active
          ? 'bg-indigo-50 text-indigo-700 shadow-sm ring-1 ring-indigo-100 dark:bg-indigo-950/45 dark:text-indigo-300 dark:ring-indigo-900/50'
    ...
    <div className="flex h-full flex-col border-e border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-950">
      ...
      <nav className="flex-1 overflow-y-auto px-2 pb-2 panel-scrollbar" aria-label="منوی مدیریت">
        {groups.map((group) => (
          <div key={group.id} className="mb-2">
            {!collapsed ? <p className="px-3 pb-2 pt-4 text-[10px] font-black tracking-[0.12em] text-slate-400 dark:text-slate-600">{group.label}</p> : <div className="py-2" aria-hidden="true" />}
            <div className="space-y-1">{group.items.map((item) => <NavLink key={item.href} item={item} active={isActive(item.href)} collapsed={collapsed} onClick={onNavigate} />)}</div>
          </div>
        ))}
      </nav>
      <div className="space-y-1 border-t border-slate-100 p-2 dark:border-slate-800">
        <Link href={`/${locale}`} onClick={onNavigate} title={collapsed ? t('nav.backToSite') : undefined} className={cn('flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-semibold text-slate-500 transition-colors hover:bg-slate-100 hover:text-slate-900 dark:text-slate-400 dark:hover:bg-slate-900 dark:hover:text-white', collapsed && 'justify-center px-2')}>
          <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-slate-100 dark:bg-slate-900"><Home className="h-4 w-4" aria-hidden="true" /></span>{!collapsed ? t('nav.backToSite') : null}
        </Link>
        <Link href="/api/auth/logout" onClick={onNavigate} title={collapsed ? t('nav.logout') : undefined} className={cn('flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-semibold text-rose-600 transition-colors hover:bg-rose-50 dark:text-rose-400 dark:hover:bg-rose-950/30', collapsed && 'justify-center px-2')}>
          <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-rose-50 dark:bg-rose-950/30"><LogOut className="h-4 w-4" aria-hidden="true" /></span>{!collapsed ? t('nav.logout') : null}
        </Link>
      </div>
    </div>
  );
...
function CommandPalette({ open, onClose, groups }: { open: boolean; onClose: () => void; groups: Group[] }) {
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-start justify-center bg-slate-950/55 p-4 pt-[12vh] backdrop-blur-sm" role="dialog" aria-modal="true" aria-label="جستجوی مدیریت" onMouseDown={(e) => { if (e.target === e.currentTarget) onClose(); }}>
...
export function AdminShell({ locale, userName, children }: AdminShellProps) {
  ...
  const items: NavItem[] = [
    ...
    { href: `${base}/products`, label: t('nav.products'), icon: Package, group: 'catalog', keywords: 'catalog product sku' },
    { href: `${base}/categories`, label: t('nav.categories'), icon: FolderTree, group: 'catalog', keywords: 'catalog category taxonomy' },
    { href: `${base}/media`, label: 'کتابخانه رسانه', icon: Images, group: 'catalog', keywords: 'media upload images files' },
    { href: `${base}/orders`, label: t('nav.orders'), icon: ShoppingBag, group: 'commerce', keywords: 'commerce orders fulfillment' },
    ...
    { href: `${base}/banners`, label: 'بنرها و کمپین‌ها', icon: Megaphone, group: 'marketing', keywords: 'banner campaign promotion advertising' },
    { href: `${base}/homepage`, label: 'سازنده صفحه اصلی', icon: PanelsTopLeft, group: 'marketing', keywords: 'homepage content merchandising' },
    { href: `${base}/marketplace`, label: 'الگوریتم پیشنهاد محصولات', icon: Sparkles, group: 'marketing', keywords: 'recommendations merchandising ranking' },
    { href: `${base}/users`, label: t('nav.users'), icon: Users, group: 'customers', keywords: 'customers users accounts' },
    { href: `${base}/sellers`, label: t('nav.sellers'), icon: Store, group: 'customers', keywords: 'vendors sellers marketplace' },
    { href: `${base}/reviews`, label: 'مدیریت نظرات', icon: Star, group: 'customers', keywords: 'reviews ratings' },
    { href: `${base}/analytics`, label: 'مرکز تحلیل', icon: BarChart3, group: 'insights', keywords: 'analytics metrics' },
    { href: `${base}/reports`, label: t('nav.reports'), icon: ReceiptText, group: 'insights', keywords: 'reports exports' },
    { href: `${base}/search`, label: 'مدیریت جستجو', icon: Search, group: 'insights', keywords: 'search discovery' },
    ...
    { href: `${base}/audit`, label: 'گزارش حسابرسی', icon: ClipboardList, group: 'system', keywords: 'audit history logs security' },
  ];
  const groups: Group[] = [
    { id: 'overview', label: 'نمای کلی', items: items.filter((i) => i.group === 'overview') },
    { id: 'catalog', label: 'کاتالوگ و محتوا', items: items.filter((i) => i.group === 'catalog') },
    { id: 'commerce', label: 'تجارت و عملیات', items: items.filter((i) => i.group === 'commerce') },
    { id: 'marketing', label: 'بازاریابی و Merchandising', items: items.filter((i) => i.group === 'marketing') },
    { id: 'customers', label: 'کاربران و فروشندگان', items: items.filter((i) => i.group === 'customers') },
    { id: 'insights', label: 'داده و بینش', items: items.filter((i) => i.group === 'insights') },
    { id: 'system', label: 'سیستم و امنیت', items: items.filter((i) => i.group === 'system') },
  ];
  ...
  const mobileSidebar = <Sidebar groups={groups} isActive={isActive} userName={userName} locale={locale} collapsed={false} onNavigate={() => setMobileOpen(false)} />;
  return (
    <div className="flex min-h-screen bg-slate-50 dark:bg-slate-950">
      <aside className={cn('hidden shrink-0 transition-[width] duration-200 md:flex md:flex-col', collapsed ? 'w-[78px]' : 'w-[272px]')}><Sidebar groups={groups} isActive={isActive} userName={userName} locale={locale} collapsed={collapsed} /></aside>
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <header className="sticky top-0 z-30 flex h-16 items-center gap-3 border-b border-slate-200 bg-white/95 px-4 shadow-sm backdrop-blur-md dark:border-slate-800 dark:bg-slate-950/95 sm:px-5">
          ...
          <button type="button" onClick={() => setCommandOpen(true)} className="flex min-w-0 flex-1 items-center gap-2 text-start sm:max-w-xl"><div className="flex h-10 min-w-0 flex-1 items-center gap-2 rounded-xl border border-slate-200 bg-slate-50 px-3 text-xs text-slate-400 hover:border-indigo-200 hover:bg-indigo-50/30 dark:border-slate-800 dark:bg-slate-900/60 dark:hover:border-indigo-900"><Command className="h-4 w-4 shrink-0" aria-hidden="true" /><span className="truncate">جستجوی سریع در کل پنل…</span><kbd className="ms-auto hidden rounded-md border border-slate-200 bg-white px-1.5 py-0.5 text-[10px] text-slate-400 dark:border-slate-700 dark:bg-slate-900 sm:block">Ctrl K</kbd></div></button>
          <div className="ms-auto flex items-center gap-1.5"><div className="hidden min-w-0 sm:block"><div className="truncate text-xs font-black">{currentItem?.label ?? 'مدیریت Empire'}</div><div className="text-[10px] text-slate-400">Commerce Admin</div></div><ThemeToggle variant="icon" lang="fa" /><Link href={`${base}/notifications`} className="relative flex h-9 w-9 items-center justify-center rounded-xl border border-slate-200 bg-white text-slate-500 shadow-sm dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300" aria-label="اعلان‌ها"><Bell className="h-4 w-4" aria-hidden="true" /><span className="absolute end-1.5 top-1.5 h-2 w-2 rounded-full bg-rose-500" aria-hidden="true" /></Link></div>
        </header>
