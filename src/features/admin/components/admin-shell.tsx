...
import {
  ...
  LogOut, Bell, Settings, Home, X, ChevronRight, Shield, Megaphone,
} from 'lucide-react';
import { cn } from '@/lib/utils';
import { ThemeToggle } from '@/components/ui/theme-toggle';

interface NavItem { href: string; label: string; icon: React.ComponentType<{ className?: string }>; group: 'main' | 'management' | 'finance'; }
...
function SidebarContent({ items, isActive, userName, locale, onNavigate }: { items: NavItem[]; isActive: (href: string) => boolean; userName: string; locale: string; onNavigate?: () => void }) {
  ...
  return <div className="flex h-full flex-col border-e border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-950">
    <div className="border-b border-gray-100 px-4 py-4 dark:border-gray-800/80"><Link href={`/${locale}`} onClick={onNavigate} className="group flex items-center gap-2.5"><div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-indigo-500 to-indigo-700 shadow-md shadow-indigo-500/25 transition-transform group-hover:scale-105"><Shield className="h-5 w-5 text-white" aria-hidden /></div><div className="flex flex-col"><p className="text-sm font-extrabold leading-none tracking-tight text-gray-900 dark:text-white">EmpireShop</p><p className="mt-0.5 text-[10px] font-bold uppercase tracking-wide text-indigo-500 dark:text-indigo-400">{t('brand')}</p></div></Link></div>
    ...
    <nav className="flex-1 overflow-y-auto px-3 py-2 panel-scrollbar" aria-label={t('brand')}>
      <NavGroup label={t('groups.main')}>{mainItems.map((item) => <NavLink key={item.href} item={item} active={isActive(item.href)} onClick={onNavigate} />)}</NavGroup>
      {mgmtItems.length > 0 && <NavGroup label={t('groups.management')}>{mgmtItems.map((item) => <NavLink key={item.href} item={item} active={isActive(item.href)} onClick={onNavigate} />)}</NavGroup>}
      {financeItems.length > 0 && <NavGroup label={t('groups.finance')}>{financeItems.map((item) => <NavLink key={item.href} item={item} active={isActive(item.href)} onClick={onNavigate} />)}</NavGroup>}
    </nav>
...
export function AdminShell({ locale, userName, children }: AdminShellProps) {
  ...
  const items: NavItem[] = [
    ...
    { href: `${base}/advertisements`, label: 'تبلیغات', icon: Megaphone, group: 'main' },
  ...
  return <div className="flex min-h-screen bg-gray-50 dark:bg-gray-950">
    ...
    {mobileOpen && <div className="fixed inset-0 z-50 md:hidden" role="dialog" aria-modal="true"><div className="absolute inset-0 bg-black/50 backdrop-blur-sm" onClick={() => setMobileOpen(false)} /><aside className="absolute inset-y-0 start-0 w-[256px] shadow-2xl"><SidebarContent items={items} isActive={isActive} userName={userName} locale={locale} onNavigate={() => setMobileOpen(false)} /></aside><button onClick={() => setMobileOpen(false)} aria-label={t('actions.cancel')} className="absolute end-4 top-4 flex h-8 w-8 items-center justify-center rounded-xl bg-white/15 text-white backdrop-blur-sm hover:bg-white/25"><X className="h-4 w-4" /></button></div>}
    <div className="flex min-w-0 flex-1 flex-col"><header className="sticky top-0 z-30 flex h-14 items-center gap-3 border-b border-gray-200 bg-white/95 px-4 shadow-sm backdrop-blur-md dark:border-gray-800 dark:bg-gray-950/95"><button className="inline-flex h-8 w-8 items-center justify-center rounded-xl border border-gray-200 text-gray-500 hover:bg-gray-100 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-800 md:hidden" aria-label={t('brand')} onClick={() => setMobileOpen(true)}><Menu className="h-4 w-4" /></button><div className="flex min-w-0 items-center gap-2 text-sm"><div className="flex items-center gap-1.5"><Shield className="h-4 w-4 shrink-0 text-indigo-500" aria-hidden /><span className="hidden font-bold text-gray-900 dark:text-gray-100 sm:block">{t('brand')}</span></div>{currentItem && <><ChevronRight className="hidden h-3.5 w-3.5 shrink-0 text-gray-300 dark:text-gray-600 sm:block rtl:rotate-180" aria-hidden /><span className="truncate text-gray-500 dark:text-gray-400">{currentItem.label}</span></>}</div><div className="flex-1" /><div className="flex items-center gap-1.5"><ThemeToggle variant="icon" lang={locale} /><button className="relative flex h-8 w-8 items-center justify-center rounded-xl border border-gray-200 text-gray-500 hover:bg-gray-100 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-800" aria-label="Notifications"><Bell className="h-4 w-4" aria-hidden /><span className="absolute end-1.5 top-1.5 h-2 w-2 rounded-full bg-red-500 ring-1 ring-white dark:ring-gray-950" aria-hidden /></button><Link href={`/${locale}/settings`} className="flex h-8 w-8 items-center justify-center rounded-xl border border-gray-200 text-gray-500 hover:bg-gray-100 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-800" aria-label={t('nav.settings')}><Settings className="h-4 w-4" aria-hidden /></Link></div></header><main id="main" className="flex-1 p-4 sm:p-6">{children}</main></div>
  </div>;
}
