...
function ImagePicker({ value, busy, onUploaded }: ImagePickerProps) {
  ...
  async function upload(file: File | undefined) {
    ...
    if (file.size <= 0 || file.size > 20 * 1024 * 1024) return toast.error('حداکثر حجم تصویر ۲۰ مگابایت است.');
    setUploading(true);
    try {
      ...
      if (!response.ok || !data?.ok || !data.data?.url) throw new Error(data?.error?.message || 'آپلود تصویر ناموفق بود.');
...
export function CategoryManager({ initial }: CategoryManagerProps) {
  ...
  async function createCategory() {
    ...
    try {
      ...
      toast.success('دسته‌بندی ایجاد شد.'); setShowNew(false); setNewDraft({ name: '', parentId: '', imageUrl: '' }); await reload(); router.refresh();
  ...
  async function remove(id: string) {
    ...
    try {
      ...
      const data = await res.json().catch(() => null) as { ok?: boolean; error?: { message?: string } | null } | null;
  ...
  return <div className="space-y-5"><div className="flex flex-col gap-4 rounded-3xl border border-border bg-muted/20 p-5 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-center gap-3"><div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-primary/10 text-primary"><FolderTree className="h-5 w-5" /></div><div><p className="text-sm font-black">دسته‌بندی فروشگاه</p><p className="mt-1 text-xs text-muted-foreground">نام، والد، تصویر و وضعیت را مدیریت کنید؛ تنظیمات فنی خودکار است.</p></div></div><div className="flex items-center gap-2"><span className="rounded-full border border-border bg-background px-3 py-1.5 text-xs font-semibold">{items.length} دسته</span><Button size="sm" onClick={() => setShowNew((v) => !v)}><Plus className="h-4 w-4" />افزودن دسته</Button></div></div>{showNew ? <Card className="p-5"><div className="grid gap-4 lg:grid-cols-[1.2fr_1fr_1.2fr]"><label className="space-y-2"><span className="text-xs font-bold">نام دسته</span><Input value={newDraft.name} onChange={(e) => setNewDraft((v) => ({ ...v, name: e.target.value }))} placeholder="مثلاً پوشاک" autoFocus /></label><label className="space-y-2"><span className="text-xs font-bold">دسته والد</span><select value={newDraft.parentId} onChange={(e) => setNewDraft((v) => ({ ...v, parentId: e.target.value }))} className="h-10 w-full rounded-md border border-input bg-background px-3 text-sm"><option value="">دسته اصلی</option>{roots.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}</select></label><div className="space-y-2"><span className="text-xs font-bold">تصویر</span><ImagePicker value={newDraft.imageUrl} busy={busy} onUploaded={(url) => setNewDraft((v) => ({ ...v, imageUrl: url }))} /></div><div className="flex gap-2 lg:col-span-3"><Button onClick={createCategory} disabled={busy || !newDraft.name.trim()}>ذخیره دسته</Button><Button variant="ghost" onClick={() => setShowNew(false)} disabled={busy}>انصراف</Button></div></div></Card> : null}<div className="overflow-x-auto rounded-2xl border border-border bg-background"><table className="w-full min-w-[760px] text-sm"><thead><tr className="border-b border-border bg-muted/50"><th className="px-4 py-3 text-start text-xs font-bold text-muted-foreground">دسته</th><th className="px-4 py-3 text-start text-xs font-bold text-muted-foreground">تصویر</th><th className="px-4 py-3 text-start text-xs font-bold text-muted-foreground">والد</th><th className="px-4 py-3 text-start text-xs font-bold text-muted-foreground">محصولات</th><th className="px-4 py-3 text-start text-xs font-bold text-muted-foreground">وضعیت</th><th className="px-4 py-3 text-end text-xs font-bold text-muted-foreground">عملیات</th></tr></thead><tbody>{items.length === 0 ? <tr><td colSpan={6} className="px-4 py-14 text-center text-sm text-muted-foreground">هنوز دسته‌ای ثبت نشده است.</td></tr> : items.map((c) => { const isEditing = editingId === c.id; return <tr key={c.id} className="border-b border-border last:border-0 hover:bg-muted/20"><td className="px-4 py-3">{isEditing ? <Input value={String(draft.name ?? '')} onChange={(e) => setDraft((v) => ({ ...v, name: e.target.value }))} /> : <div><p className="font-bold">{c.name}</p><p className="mt-0.5 text-xs text-muted-foreground">تنظیمات فنی خودکار</p></div>}</td><td className="px-4 py-3">{isEditing ? <ImagePicker value={String(draft.imageUrl ?? '') || null} busy={busy} onUploaded={(url) => setDraft((v) => ({ ...v, imageUrl: url }))} /> : c.imageUrl ? <img src={c.imageUrl} alt="" loading="lazy" className="h-11 w-16 rounded-xl border border-border object-cover" /> : <div className="flex h-11 w-16 items-center justify-center rounded-xl bg-muted text-muted-foreground"><ImageIcon className="h-4 w-4" /></div>}</td><td className="px-4 py-3">{isEditing ? <select value={String(draft.parentId ?? '')} onChange={(e) => setDraft((v) => ({ ...v, parentId: e.target.value || null }))} className="h-9 min-w-36 rounded-md border border-input bg-background px-2 text-xs"><option value="">دسته اصلی</option>{roots.filter((root) => root.id !== c.id).map((root) => <option key={root.id} value={root.id}>{root.name}</option>)}</select> : parentName(c.parentId)}</td><td className="px-4 py-3 font-semibold">{c.productCount.toLocaleString('fa-AF')}</td><td className="px-4 py-3">{isEditing ? <label className="inline-flex items-center gap-2 text-xs"><input type="checkbox" checked={draft.isActive !== false} onChange={(e) => setDraft((v) => ({ ...v, isActive: e.target.checked }))} />فعال</label> : <span className={`inline-flex rounded-full px-2.5 py-1 text-xs font-bold ${c.isActive === false ? 'bg-muted text-muted-foreground' : 'bg-emerald-500/10 text-emerald-700 dark:text-emerald-400'}`}>{c.isActive === false ? 'غیرفعال' : 'فعال'}</span>}</td><td className="px-4 py-3"><div className="flex justify-end gap-2">{isEditing ? <><button type="button" aria-label="ذخیره" onClick={() => saveEdit(c.id)} disabled={busy} className="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-border text-emerald-600 hover:bg-emerald-500/10"><Check className="h-3.5 w-3.5" /></button><button type="button" aria-label="انصراف" onClick={() => setEditingId(null)} disabled={busy} className="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-border text-muted-foreground hover:bg-muted"><X className="h-3.5 w-3.5" /></button></> : <><button type="button" aria-label="ویرایش" onClick={() => { setEditingId(c.id); setDraft({ ...c }); }} className="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-border text-muted-foreground hover:bg-muted"><Pencil className="h-3.5 w-3.5" /></button><button type="button" aria-label="حذف" onClick={() => remove(c.id)} disabled={busy} className="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-border text-red-600 hover:bg-red-500/10"><Trash2 className="h-3.5 w-3.5" /></button></>}</div></td></tr>; })}</tbody></table></div></div>;
}
