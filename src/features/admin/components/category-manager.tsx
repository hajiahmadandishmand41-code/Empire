    ...
    try {
      const res = await fetch('/api/admin/categories', {
        ...
        body: JSON.stringify({
          ...
          parentId: newDraft.parentId || null,
  ...
  return (
    <div className="space-y-5">
      ...
      {showNew && <Card className="p-5"><div className="grid gap-4 md:grid-cols-2 lg:grid-cols-5">
        <label className="space-y-2 lg:col-span-1"><span className="text-xs font-bold">نام دسته</span><Input value={newDraft.name} onChange={(e) => setNewDraft((v) => ({ ...v, name: e.target.value }))} /></label>
        ...
        <label className="space-y-2"><span className="text-xs font-bold">URL تصویر</span><Input dir="ltr" value={newDraft.imageUrl} onChange={(e) => setNewDraft((v) => ({ ...v, imageUrl: e.target.value }))} placeholder="https://..." /></label>
      ...
      <div className="overflow-x-auto rounded-2xl border border-border bg-background">
        <table className="w-full min-w-[980px] text-sm">
          ...
          <tbody>
            {items.length === 0 ? <tr><td colSpan={7} className="px-4 py-14 text-center text-sm text-muted-foreground">هنوز دسته‌بندی‌ای ثبت نشده است.</td></tr> : items.map((c) => {
              const isEditing = editingId === c.id;
              return <tr key={c.id} className="border-b border-border last:border-0 hover:bg-muted/20">
                <td className="px-4 py-3">{isEditing ? <div className="space-y-2"><Input value={String(draft.name ?? '')} onChange={(e) => setDraft((v) => ({ ...v, name: e.target.value }))} /><Input dir="ltr" value={String(draft.imageUrl ?? '')} onChange={(e) => setDraft((v) => ({ ...v, imageUrl: e.target.value }))} placeholder="Image URL" /></div> : <div><p className="font-bold">{c.name}</p><p className="mt-0.5 text-xs text-muted-foreground">{c.slug}</p></div>}</td>
