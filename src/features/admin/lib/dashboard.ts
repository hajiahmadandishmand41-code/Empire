...
export async function getAdminDashboardMetrics() {
  ...
  const [
    ...
    prisma.$queryRawUnsafe<Array<any>>(
      `SELECT COALESCE(SUM(CASE WHEN o."createdAt">=CURRENT_DATE AND o.status<>'cancelled' THEN o.total ELSE 0 END),0) AS today,
    ...
    prisma.$queryRawUnsafe<Array<any>>(
      `SELECT TO_CHAR(DATE_TRUNC('day',"createdAt"),'YYYY-MM-DD') AS date,
       ...
       FROM "Order"
       WHERE "createdAt">=CURRENT_DATE-INTERVAL '29 day'
       GROUP BY 1 ORDER BY 1 ASC`,
    ),
    prisma.product.findMany({
      where: { isActive: false },
      select: { id: true, name: true, stockQuantity: true, sellerId: true, createdAt: true },
    ...
    prisma.$queryRawUnsafe<Array<any>>(
      `SELECT COALESCE(u."sellerShopName",u."fullName") AS name,
       ...
       GROUP BY u.id,u."sellerShopName",u."fullName"
