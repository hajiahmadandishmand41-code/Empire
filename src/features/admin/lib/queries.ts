...
export async function listAdminProducts(f: ListFilters = {}): Promise<Paged<AdminProductRow>> {
  ...
  try { const where: Prisma.ProductWhereInput = q ? { OR: [{ name: { contains: q, mode: 'insensitive' } }, { slug: { contains: q, mode: 'insensitive' } }, { region: { contains: q, mode: 'insensitive' } }] } : {}; const [rows, total] = await Promise.all([prisma.product.findMany({ where, include: { category: true }, orderBy: { createdAt: 'desc' }, take: pageSize, skip: (page - 1) * pageSize }), prisma.product.count({ where })]); if (isDev && total === 0 && !q) return paginate(mockProducts, page, pageSize); return { items: rows.map((p) => ({ id: p.id, slug: p.slug, name: p.name, price: p.price.toNumber(), currency: p.currency, categoryName: p.category.name, region: p.region, inStock: p.inStock, createdAt: p.createdAt.toISOString() })), total, page, pageSize, source: 'db' }; }
