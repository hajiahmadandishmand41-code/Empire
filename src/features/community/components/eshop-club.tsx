...
export function EshopClub({ locale }: { locale: Locale }) {
  ...
  const submit = async (action: 'create' | 'reply', postId?: string) => {
    ...
    try {
      ...
      if (action === 'create') { setMessage(''); setComposerOpen(false); } else setReplyText((state) => ({ ...state, [postId ?? '']: '' }));
  ...
  return (
    <div className="min-h-dvh bg-background">
      <section className="relative overflow-hidden border-b border-border bg-gradient-to-br from-rose-950 via-indigo-950 to-slate-950 text-white">
        <div className="absolute -left-12 top-4 h-24 w-24 rounded-full bg-rose-400/10 blur-2xl" />
        <div className="absolute -right-12 bottom-0 h-28 w-28 rounded-full bg-violet-400/8 blur-2xl" />
        <div className="relative mx-auto max-w-screen-xl px-3 py-3 sm:px-6 sm:py-4">
          <div className="flex items-center justify-between gap-3">
            <div className="min-w-0">
              <span className="inline-flex items-center gap-1 rounded-full border border-white/15 bg-white/10 px-2 py-0.5 text-[8px] font-extrabold text-rose-100"><Sparkles className="h-2.5 w-2.5" />{t.eyebrow}</span>
              <h1 className="mt-1 text-lg font-black tracking-tight sm:text-2xl">{t.title}</h1>
              <p className="mt-0.5 line-clamp-1 max-w-xl text-[9px] leading-4 text-white/65 sm:text-xs">{t.subtitle}</p>
            </div>
            <div className="hidden shrink-0 items-center gap-1.5 rounded-full border border-white/10 bg-white/10 px-2.5 py-1.5 text-[8px] font-bold text-white/75 sm:flex"><UsersRound className="h-3 w-3" />{t.community}</div>
          </div>
        </div>
      </section>

      <main className="mx-auto grid max-w-screen-xl gap-4 px-3 py-3 sm:px-6 sm:py-5 lg:grid-cols-[minmax(0,1fr)_260px]">
        <section className="min-w-0">
          <div className="rounded-2xl border border-border bg-card p-2.5 shadow-sm sm:p-3">
            {!composerOpen ? (
              <button type="button" onClick={() => setComposerOpen(true)} className="flex min-h-11 w-full items-center gap-2.5 rounded-xl border border-dashed border-border bg-background px-3 text-start transition hover:border-primary/30 hover:bg-primary/5">
                <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-primary/10 text-primary"><Plus className="h-4 w-4" /></span>
                <span className="min-w-0 flex-1 text-xs font-bold text-muted-foreground">{t.write}</span>
              ...
              <div className="flex items-start gap-2.5">
                ...
                <div className="min-w-0 flex-1">
                  <textarea autoFocus value={message} onChange={(e) => setMessage(e.target.value.slice(0, 1200))} rows={2} placeholder={t.placeholder} className="w-full resize-none rounded-xl border border-border bg-background px-3 py-2.5 text-xs leading-5 outline-none transition focus:border-primary/40 focus:ring-2 focus:ring-primary/10" />
                  <div className="mt-2 flex items-center justify-between gap-2">
                    <button type="button" onClick={() => setComposerOpen(false)} className="text-[9px] font-bold text-muted-foreground hover:text-foreground">×</button>
                    <div className="flex items-center gap-2"><span className="text-[9px] text-muted-foreground">{message.length}/1200</span><button type="button" aria-label={t.send} title={t.send} onClick={() => void submit('create')} disabled={!message.trim() || posting} className="flex h-9 items-center justify-center gap-1.5 rounded-full bg-primary px-3 text-[10px] font-bold text-primary-foreground shadow-sm transition hover:-translate-y-0.5 hover:shadow-md disabled:cursor-not-allowed disabled:opacity-50">{posting ? <Loader2 className="h-4 w-4 animate-spin" /> : <Send className="h-3.5 w-3.5" />}{t.send}</button></div>
                  </div>
                </div>
              </div>
            )}
