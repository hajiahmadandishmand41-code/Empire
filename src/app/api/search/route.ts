/** GET /api/search — database-backed search with analytics. */
...
const searchQuerySchema = z.object({
  ...
  page: z.coerce.number().int().positive().max(1000).optional(), pageSize: z.coerce.number().int().positive().max(100).optional(),
});
  ...
  try {
    ...
    try {
      await prisma.$executeRawUnsafe(
        `INSERT INTO "SearchQueryStat" ("id","query","resultCount","searchCount","lastSearchedAt") VALUES (gen_random_uuid()::text,$1,$2,1,NOW())
         ON CONFLICT ("query") DO UPDATE SET "resultCount"=$2,"searchCount"="SearchQueryStat"."searchCount"+1,"lastSearchedAt"=NOW()`,
        q.toLowerCase(), result.total,
      );
    } catch {
      // Analytics must never make a successful search fail.
    }
    return jsonOk(result.products, { meta: { source: 'db', query: result.meta.query, total: result.total, page: result.page, pageSize: result.pageSize, hasMore: result.hasMore, durationMs: result.meta.durationMs, reranked: result.meta.reranked } });
  } catch (err) {
