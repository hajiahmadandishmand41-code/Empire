...
export async function GET(){const g=await requireAdminApi('notifications.manage');if(!g.ok)return g.response;try{return jsonOk(await prisma.$queryRawUnsafe<Array<any>>('SELECT * FROM "AdminNotification" ORDER BY "createdAt" DESC LIMIT 100'));}catch{return jsonError('db_unavailable','Notifications unavailable',{status:503});}}
