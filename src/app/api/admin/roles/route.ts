...
export async function PUT(req:NextRequest){ const guard=await requireAdminApi('users.manage'); if(!guard.ok) return guard.response; if(guard.accessRole!=='super_admin') return jsonError('forbidden','Only a super admin can change admin roles',{status:403}); const p=schema.safeParse(await req.json().catch(()=>null)); if(!p.success) return jsonError('invalid_body','Invalid access role payload',{status:422}); if(p.data.userId===guard.user.id && p.data.accessRole!=='super_admin') return jsonError('protected_admin','You cannot demote your own admin access',{status:403}); try { await upsertAdminAccess(p.data.userId,p.data.accessRole as AdminAccessRole,p.data.permissions??DEFAULT_PERMISSIONS[p.data.accessRole as AdminAccessRole]); return jsonOk({saved:true}); } catch { return jsonError('save_failed','Failed to save role',{status:500}); } }
