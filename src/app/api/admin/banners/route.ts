...
const schema = z.object({
  ...
  if (value.startAt && value.endAt && new Date(value.endAt) <= new Date(value.startAt)) {
    ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['endAt'], message: 'End time must be after start time' });
  }
});

export async function GET() {
  const g = await requireAdminApi('banners.manage');
  if (!g.ok) return g.response;
  try { return jsonOk(await listAllBanners()); }
  catch { return jsonError('query_failed', 'Failed to load banners', { status: 500 }); }
}
  ...
  try {
    ...
    if (d.id) {
      ...
      await prisma.$executeRaw`
        UPDATE "Banner"
        SET "key" = ${d.key}, "placement" = ${d.placement}, "title" = ${d.title ?? null}, "subtitle" = ${d.subtitle ?? null},
            "ctaLabel" = ${d.ctaLabel ?? null}, "href" = ${d.href ?? null}, "desktopImageUrl" = ${d.desktopImageUrl},
            "mobileImageUrl" = ${d.mobileImageUrl ?? null}, "startAt" = ${d.startAt ? new Date(d.startAt) : null},
            "endAt" = ${d.endAt ? new Date(d.endAt) : null}, "sortOrder" = ${d.sortOrder}, "autoSlide" = ${d.autoSlide},
            "durationMs" = ${d.durationMs}, "isActive" = ${d.isActive}, "updatedAt" = NOW()
        WHERE "id" = ${d.id}
      `;
    ...
    await prisma.$executeRaw`
      ...
      VALUES (${id},${d.key},${d.placement},${d.title ?? null},${d.subtitle ?? null},${d.ctaLabel ?? null},${d.href ?? null},${d.desktopImageUrl},${d.mobileImageUrl ?? null},${d.startAt ? new Date(d.startAt) : null},${d.endAt ? new Date(d.endAt) : null},${d.sortOrder},${d.autoSlide},${d.durationMs},${d.isActive},NOW(),NOW())
      ON CONFLICT ("key") DO UPDATE SET "placement"=EXCLUDED."placement","title"=EXCLUDED."title","subtitle"=EXCLUDED."subtitle","ctaLabel"=EXCLUDED."ctaLabel","href"=EXCLUDED."href","desktopImageUrl"=EXCLUDED."desktopImageUrl","mobileImageUrl"=EXCLUDED."mobileImageUrl","startAt"=EXCLUDED."startAt","endAt"=EXCLUDED."endAt","sortOrder"=EXCLUDED."sortOrder","autoSlide"=EXCLUDED."autoSlide","durationMs"=EXCLUDED."durationMs","isActive"=EXCLUDED."isActive","updatedAt"=NOW()
    `;
...
export async function DELETE(req: NextRequest) {
  ...
  try {
    const result = await prisma.$executeRaw`DELETE FROM "Banner" WHERE "id" = ${id}`;
    if (Number(result) === 0) return jsonError('not_found', 'Banner not found', { status: 404 });
