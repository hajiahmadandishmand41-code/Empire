...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  const { id } = await params; const g = await requireAdminApi('products.manage'); if (!g.ok) return g.response;
  let body: unknown; try { body = await req.json(); } catch { return jsonError('invalid_json', 'Invalid JSON', { status: 400 }); }
  const p = patchSchema.safeParse(body); if (!p.success) return jsonError('invalid_body', 'Invalid product patch', { status: 422, details: { issues: p.error.issues } });
  if (!isDatabaseConfigured()) return jsonError('db_unavailable', 'Database is not configured', { status: 503 });
  try {
    const before = await prisma.product.findUnique({ where: { id } }); if (!before) return jsonError('not_found', 'Product not found', { status: 404 });
    const nextImages = p.data.imagesJson ?? normalizeImageUrls(before.imagesJson); const nextPrimary = p.data.primaryImageIndex ?? before.primaryImageIndex;
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!isDatabaseConfigured()) return jsonError('db_unavailable', 'Database is not configured', { status: 503 });
  try {
    const product = await prisma.product.findUnique({ where: { id }, select: { id: true, isActive: true, inStock: true } }); if (!product) return jsonError('not_found', 'Product not found', { status: 404 });
    const orderCount = await prisma.orderItem.count({ where: { productId: id } });
    if (orderCount > 0) { const after = await prisma.product.update({ where: { id }, data: { isActive: false, inStock: false } }); await recordAudit({ actor: { id: g.user.id, role: g.accessRole }, action: 'product.archive', entityType: 'product', entityId: id, before: product, after, metadata: { orderCount }, req }); return jsonOk({ deleted: false, archived: true, reason: 'product_has_order_history' }); }
    await prisma.product.delete({ where: { id } }); await recordAudit({ actor: { id: g.user.id, role: g.accessRole }, action: 'product.delete', entityType: 'product', entityId: id, before: product, after: null, req }); return jsonOk({ deleted: true });
