  ...
  try {
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (goingToCancel && previous.status === 'delivered') {
        throw new OrderTransitionError('invalid_transition', 'A delivered order cannot be cancelled — issue a refund instead.');
      }
      // There is no provider refund operation in the current payment adapter.
      // Never cancel a paid online order while leaving the customer charged.
      if (goingToCancel && previous.paymentMethod !== 'cod' && previous.paymentStatus === 'paid') {
        throw new OrderTransitionError('refund_required', 'A paid online order cannot be cancelled until a provider refund is completed.');
      }
      if (parsed.data.status === 'delivered' && previous.paymentStatus !== 'paid' && !settlingCod) {
        throw new OrderTransitionError('payment_required', 'Only paid orders can be marked as delivered. COD is settled at delivery.');
      }
    ...
    if (updated.status === 'delivered') creditedSellerLines = await creditSellersForOrder(updated.id);
