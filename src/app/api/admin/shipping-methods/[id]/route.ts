...
export async function PUT(req: NextRequest, { params }: Ctx) {
  ...
  const guard = await requireAdminApi('shipping.manage');
  ...
  if (!parsed.success) return jsonError('invalid_body', 'Invalid data', { status: 422, details: { issues: parsed.error.issues } });
...
export async function DELETE(_req: NextRequest, { params }: Ctx) {
  ...
  if (inUse > 0) {
    const updated = await prisma.shippingMethod.update({ where: { id }, data: { isActive: false } });
