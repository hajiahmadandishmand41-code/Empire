  ...
  try {
    const posts = await prisma.$queryRaw<PostRow[]>`
      SELECT
        ...
        ${userId ? prisma.$queryRaw`EXISTS(SELECT 1 FROM "CommunityPostLike" ml WHERE ml."postId" = p."id" AND ml."userId" = ${userId})` : prisma.$queryRaw`false`} AS "liked"
    ...
    return jsonOk({
      posts: posts.map((post) => ({
        ...
        replies: (replyMap.get(post.id) ?? []).map((reply) => ({
          id: reply.id,
  ...
  try {
    if (action === 'like') {
      ...
      const existing = await prisma.$queryRaw<{ id: string }[]>`
        SELECT "id" FROM "CommunityPostLike" WHERE "userId" = ${user.id} AND "postId" = ${input.postId} LIMIT 1
      `;
      if (existing.length) {
    ...
    if (action !== 'create' && action !== 'reply') return jsonError('invalid_action', 'Invalid community action', { status: 400, req });
    ...
    await prisma.$executeRaw`
      INSERT INTO "CommunityPost" ("id", "userId", "parentId", "content", "createdAt", "updatedAt")
      VALUES (${id}, ${user.id}, ${parentId}, ${content}, NOW(), NOW())
    `;
