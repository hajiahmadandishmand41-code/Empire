...
function jsonProviderValue(value: unknown): Prisma.InputJsonValue | typeof Prisma.JsonNull { if (value === null || value === undefined) return Prisma.JsonNull; return JSON.parse(JSON.stringify(value)) as Prisma.InputJsonValue; }
...
export async function POST(req: NextRequest) {
  ...
  let raw: unknown; try { raw = await req.json(); } catch { return jsonError('invalid_json', 'Request body is not valid JSON', { status: 400 }); }
  const parsed = bodySchema.safeParse(raw); if (!parsed.success) return jsonError('invalid_body', 'Invalid payload', { status: 422, details: { issues: parsed.error.issues } });
  try {
    ...
    if (order.userId) { if (!currentUser || currentUser.id !== order.userId) return jsonError('forbidden', 'Not allowed to pay for this order', { status: 403 }); }
    else if (currentUser && currentUser.role !== 'admin') return jsonError('forbidden', 'Not allowed to pay for this order', { status: 403 });
    ...
    const txn = existing ?? await prisma.transaction.create({ data: { orderId: order.id, reference: `TXN-${order.reference}-${Date.now().toString(36).toUpperCase()}`, provider: order.paymentMethod, method: order.paymentMethod, status: 'pending', amount: order.total, currency: order.currency } });
    ...
    if (order.paymentMethod === 'atoma_pay') {
      ...
      await prisma.transaction.update({ where: { id: txn.id }, data: { providerTxnId: session.providerTxnId, providerRaw: jsonProviderValue(session.raw) } });
  ...
  } catch (err) { logger.error('payments.initiate_failed', {}, err); return jsonError('payment_init_failed', 'Failed to initiate payment', { status: 500 }); }
}
