...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    return jsonOk(result.items, { meta: { total: result.total, page: result.page, pageSize: result.pageSize, source: result.source } });
  } catch (err) {
    logger.error('seller.products.list_failed', { userId: guard.user.id }, err);
