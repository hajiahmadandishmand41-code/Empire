...
export async function POST(req: NextRequest) {
  ...
  if (brandId && !(await validateSellerBrand(guard.user.id, brandId))) return jsonError('invalid_brand', 'برند انتخاب‌شده متعلق به این فروشگاه نیست یا فعال نیست.', { status: 422 });
  try {
    ...
    const actualBrand = await prisma.$queryRaw<Array<{ id: string }>>(Prisma.sql`SELECT "brandId" AS "id" FROM "Product" WHERE "id"=${created.id} LIMIT 1`);
    return jsonOk({ ...serializeProduct(created), brandId: actualBrand[0]?.id ?? null }, { status: 201 });
