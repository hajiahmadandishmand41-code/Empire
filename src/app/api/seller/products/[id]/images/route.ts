...
const EXT: Record<string, string> = {
  ...
  'image/heic': 'heic', 'image/heif': 'heif', 'image/jxl': 'jxl', 'image/apng': 'png',
};
const SAFE_ID = /^[A-Za-z0-9_-]{1,64}$/;
const jsonPostSchema = z.object({ dataUrl: z.string().min(20), alt: z.string().max(200).optional(), fileName: z.string().max(255).optional() });
...
async function loadOwned(id: string, role: string, userId: string) {
  const product = await prisma.product.findUnique({ where: { id }, select: { id: true, sellerId: true, imagesJson: true, primaryImageIndex: true, name: true } });
  if (!product) return { ok: false as const, status: 404 };
  if (role !== 'admin' && product.sellerId !== userId) return { ok: false as const, status: 403 };
...
async function cleanup(url: string, productId: string, userId: string) {
  try { await deletePersistent(url); }
  catch (error) { logger.warn('seller.products.images.cleanup_failed', { productId, userId }, error); }
}

async function readUpload(req: NextRequest): Promise<{ file: File; alt?: string } | { error: Response }> {
  const contentType = req.headers.get('content-type') ?? '';
  if (contentType.includes('multipart/form-data')) {
    let form: FormData;
    try { form = await req.formData(); } catch { return { error: jsonError('invalid_form', 'اطلاعات فایل نامعتبر است.', { status: 400 }) }; }
    const value = form.get('file');
    const altValue = form.get('alt');
    if (!(value instanceof File)) return { error: jsonError('invalid_file', 'فایل تصویر ارسال نشده است.', { status: 422 }) };
    return { file: value, alt: typeof altValue === 'string' ? altValue.slice(0, 200) : undefined };
  }

  let body: unknown;
  try { body = await req.json(); } catch { return { error: jsonError('invalid_json', 'اطلاعات ارسال‌شده نامعتبر است.', { status: 400 }) }; }
  const parsed = jsonPostSchema.safeParse(body);
  if (!parsed.success) return { error: jsonError('invalid_body', 'اطلاعات تصویر نامعتبر است.', { status: 422, details: { issues: parsed.error.issues } }) };
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  let publicUrl = '';
  try {
    const uploaded = await uploadPersistent(new File([buffer], `product.${EXT[detectedMime]}`, { type: detectedMime }), `products/${id}`);
    publicUrl = uploaded.secure_url ?? '';
    if (!publicUrl) throw new Error('storage_no_url');
  } catch (error) {
    logger.error('seller.products.images.write_failed', { productId: id, userId: guard.user.id }, error);
    return jsonError('storage_failed', 'ذخیره تصویر ناموفق بود.', { status: 503 });
  }
  ...
  for (let attempt = 1; attempt <= MAX_TRANSACTION_RETRIES; attempt += 1) {
    try {
      ...
      terminalError = error;
      if (!isSerializableConflict(error) || attempt === MAX_TRANSACTION_RETRIES) break;
    }
  }
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  catch (error) { logger.error('seller.products.images.metadata_delete_failed', { productId: id, userId: guard.user.id }, error); return jsonError('update_failed', 'حذف تصویر ناموفق بود.', { status: 500 }); }
