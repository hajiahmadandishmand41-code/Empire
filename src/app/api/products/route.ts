/** GET /api/products — public database-backed product listing. */
...
export async function GET(req: NextRequest) {
  ...
  const parsed = productListQuerySchema.safeParse(Object.fromEntries(req.nextUrl.searchParams.entries()));
  if (!parsed.success) return jsonError('invalid_query', 'Invalid query parameters', { status: 400, details: { issues: parsed.error.issues } });
  ...
  if (!isDatabaseConfigured()) return jsonError('db_unavailable', 'Product catalog is unavailable', { status: 503 });
  try {
    ...
    return jsonOk(products, { meta: { source: result.source, locale, total: result.total, page: result.page, pageSize: result.pageSize, hasMore: result.hasMore } });
  } catch (err) {
