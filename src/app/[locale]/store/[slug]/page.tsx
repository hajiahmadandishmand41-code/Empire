...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!seller) return { title: 'Eshop Store' };
...
export default async function SellerStorefront({ params }: Props) {
  ...
  const toCard = (product: StoreProductRow) => {
    const image = Array.isArray(product.imagesJson) ? (product.imagesJson as ProductImage[])[0] : null;
    return {
      id: product.id,
      name: product.name,
  ...
  return <div className="min-h-dvh bg-background"><SiteHeader/><main id="main">
    ...
    <Container size="xl" className="py-8 sm:py-10">
      {categories.length>0&&<section className="mb-10"><div className="mb-4 flex items-end justify-between"><div><p className="text-xs font-bold text-primary">فروشگاه</p><h2 className="text-xl font-black">دسته‌های این فروشگاه</h2></div><span className="text-xs text-muted-foreground">{categories.length} دسته</span></div><div className="flex gap-2 overflow-x-auto pb-2">{categories.map(category=><Link key={category.id} href={`/category/${category.slug}`} className="shrink-0 rounded-2xl border border-border bg-card px-4 py-3 text-xs font-bold transition hover:border-primary/40 hover:bg-primary/5">{category.name}</Link>)}</div></section>}
