  ...
  return (
    <div dir={direction} className="relative flex min-h-dvh flex-col items-center justify-center overflow-hidden bg-background px-4 py-8 sm:py-10">
      <div aria-hidden="true" className="pointer-events-none fixed inset-0 overflow-hidden">
        ...
        <div className="absolute -end-32 bottom-0 h-72 w-72 rounded-full bg-accent/20 blur-3xl" />
      </div>

      <Link
        href={`/${locale}`}
        className="group relative mb-7 flex items-center gap-2.5 rounded-xl focus-visible:outline-none"
        aria-label={copy.home}
      >
        <span className="flex h-10 w-10 items-center justify-center rounded-2xl bg-primary shadow-lg transition-transform group-hover:scale-105" aria-hidden="true">
          <EmpireLogo size={30} variant="color" />
        </span>
        <span className="font-display text-lg font-extrabold tracking-tight text-foreground">EmpireShop</span>
      </Link>

      <div className="relative w-full max-w-md">{children}</div>

      <p className="relative mt-7 text-xs text-muted-foreground">
        © {new Date().getFullYear()} EmpireShop · {copy.rights}
      </p>
    </div>
  );
}
