...
export async function generateMetadata({ params }: { params: Promise<{ locale: string; slug: string }> }): Promise<Metadata> {
  ...
  return { title, description, alternates: { canonical: canonicalUrl, languages: { fa: `${SITE_URL}/fa/shop/${slug}`, ps: `${SITE_URL}/ps/shop/${slug}`, en: `${SITE_URL}/en/shop/${slug}` } }, openGraph: { title, description, type: 'website', url: canonicalUrl, images: [{ url: ogImage, width: 800, height: 800, alt: product.name }], siteName: 'Eshop' }, twitter: { card: 'summary_large_image', title, description, images: [ogImage] } };
}
...
export default async function ProductPage({ params }: { params: Promise<{ locale: string; slug: string }> }) {
  ...
  const productSchema = { '@context': 'https://schema.org', '@type': 'Product', name: product.name, description: product.shortDescription ?? product.name, image: product.images.map((image) => image.src).filter(Boolean), sku: product.slug, brand: { '@type': 'Brand', name: product.sellerShopName ?? 'Eshop' }, offers: { '@type': 'Offer', url: productUrl, priceCurrency: 'AFN', price: String(product.price), availability: product.inStock ? 'https://schema.org/InStock' : 'https://schema.org/OutOfStock', seller: { '@type': 'Organization', name: product.sellerShopName ?? 'Eshop' } }, ...(reviewCount > 0 ? { aggregateRating: { '@type': 'AggregateRating', ratingValue: String(averageRating), reviewCount: String(reviewCount) } } : {}) };
  return <><SiteHeader /><main id="main" className="min-h-dvh bg-background"><RecentlyViewedTracker slug={product.slug} categoryKey={product.categoryKey} /><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(productSchema).replace(/</g, '\\u003c') }} /><ProductDetail product={product} related={related} locale={locale} currency="AFN" /></main><SiteFooter /></>;
}
