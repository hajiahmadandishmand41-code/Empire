      ...
      <main id="main" className="min-h-dvh pb-16 md:pb-0">
        ...
        <section aria-label={locale === 'en' ? 'Marketplace shortcuts' : locale === 'ps' ? 'د بازار لنډې لارې' : 'مسیرهای اصلی بازار'} className="border-b border-border bg-card">
          <div className="mx-auto grid max-w-screen-xl grid-cols-3 gap-2 px-3 py-3 sm:gap-3 sm:px-6 sm:py-4">
            <Link href="/categories" className="group flex min-h-16 items-center justify-center gap-2 rounded-2xl border border-border bg-background px-2 py-2.5 text-center text-[11px] font-extrabold text-foreground transition hover:border-primary/30 hover:bg-primary/5 sm:min-h-20 sm:text-xs"><span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-primary/10 text-primary sm:h-10 sm:w-10"><LayoutGrid className="h-4 w-4 sm:h-5 sm:w-5" aria-hidden /></span><span className="leading-4">{quickLinks.categories}</span></Link>
            <Link href="/stores" className="group flex min-h-16 items-center justify-center gap-2 rounded-2xl border border-border bg-background px-2 py-2.5 text-center text-[11px] font-extrabold text-foreground transition hover:border-primary/30 hover:bg-primary/5 sm:min-h-20 sm:text-xs"><span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-primary/10 text-primary sm:h-10 sm:w-10"><Store className="h-4 w-4 sm:h-5 sm:w-5" aria-hidden /></span><span className="leading-4">{quickLinks.stores}</span></Link>
            <Link href="/traditional" className="group flex min-h-16 items-center justify-center gap-2 rounded-2xl border border-border bg-background px-2 py-2.5 text-center text-[11px] font-extrabold text-foreground transition hover:border-primary/30 hover:bg-primary/5 sm:min-h-20 sm:text-xs"><span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-primary/10 text-primary sm:h-10 sm:w-10"><Sparkles className="h-4 w-4 sm:h-5 sm:w-5" aria-hidden /></span><span className="leading-4">{quickLinks.traditional}</span></Link>
          </div>
        </section>
        ...
        <Suspense fallback={<SectionSkeleton />}><HomeProductSection section="featured" locale={locale} title={{ en: 'Special offers', ps: 'ځانګړي وړاندیزونه', fa: 'پیشنهادهای ویژه' }} subtitle={{ en: 'Limited-time products worth checking today', ps: 'د نن ورځې محدود او ارزښتناک وړاندیزونه', fa: 'محصولات محدود و ارزشمند امروز' }} href="/shop?badge=sale" badge="featured" /></Suspense>
        <Suspense fallback={<SectionSkeleton />}><HomeProductSection section="bestSelling" locale={locale} title={{ en: 'Best sellers', ps: 'تر ټولو ډېر پلورل شوي', fa: 'پرفروش‌ترین‌ها' }} href="/shop?sort=bestSelling" badge="best" accentColor="bg-amber-500" /></Suspense>
        <DynamicBannerStrip locale={locale} placement="HOME_PROMO_1" />
        <Suspense fallback={<SectionSkeleton />}><HomeProductSection section="newest" locale={locale} title={{ en: 'New arrivals', ps: 'نوي محصولات', fa: 'جدیدترین‌ها' }} href="/shop?sort=newest" badge="new" accentColor="bg-sky-500" /></Suspense>
        <Suspense fallback={<SectionSkeleton />}><HomeProductSection section="popular" locale={locale} title={{ en: 'Popular products', ps: 'مشهور محصولات', fa: 'محبوب‌ترین محصولات' }} href="/shop?sort=popular" badge="popular" accentColor="bg-fuchsia-500" /></Suspense>
