...
function HomeDiscoveryLinks({ locale }: { locale: Locale }) {
  const labels = locale === 'en'
    ? { discover: 'Explore', deals: 'Deals', new: 'New arrivals', stores: 'Stores', local: 'Local products' }
    : locale === 'ps'
      ? { discover: 'کشف', deals: 'ځانګړي وړاندیزونه', new: 'نوي محصولات', stores: 'پلورنځي', local: 'کورني محصولات' }
      : { discover: 'کشف کنید', deals: 'پیشنهادها', new: 'جدیدترین‌ها', stores: 'فروشگاه‌ها', local: 'محصولات وطنی' };

  const items = [
    { href: '/discover', label: labels.discover, icon: Compass, tone: 'bg-primary/10 text-primary' },
    { href: '/shop?badge=sale', label: labels.deals, icon: Tag, tone: 'bg-amber-500/10 text-amber-600 dark:text-amber-400' },
    { href: '/shop?sort=newest', label: labels.new, icon: PackagePlus, tone: 'bg-sky-500/10 text-sky-600 dark:text-sky-400' },
    { href: '/stores', label: labels.stores, icon: Store, tone: 'bg-violet-500/10 text-violet-600 dark:text-violet-400' },
  ];

  return (
    <section aria-label={locale === 'en' ? 'Explore marketplace' : locale === 'ps' ? 'بازار وپلټئ' : 'کشف بازار'} className="border-b border-border bg-card">
      <div className="mx-auto grid max-w-screen-xl grid-cols-2 gap-2.5 px-3 py-3 sm:grid-cols-4 sm:gap-3 sm:px-6 sm:py-4">
        {items.map(({ href, label, icon: Icon, tone }) => (
          <Link key={href} href={href as never} className="group flex min-h-16 items-center gap-2.5 rounded-2xl border border-border bg-background px-3 py-2.5 transition-[border-color,transform,box-shadow,background-color] duration-200 hover:-translate-y-0.5 hover:border-primary/25 hover:bg-primary/[0.03] hover:shadow-sm sm:min-h-20 sm:justify-center sm:px-4">
            <span className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-xl ${tone}`}><Icon className="h-4.5 w-4.5" aria-hidden /></span>
            <span className="text-[11px] font-extrabold leading-4 text-foreground sm:text-xs">{label}</span>
          </Link>
        ))}
      </div>
    </section>
  ...
  return (
    <div className="min-h-dvh bg-background">
      ...
      <main id="main" className="min-h-dvh pb-16 md:pb-0">
        ...
        <HomeDiscoveryLinks locale={locale} />
