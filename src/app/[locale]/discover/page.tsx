    ...
    <section id="stores" aria-labelledby="stores-title" className="mx-auto max-w-screen-xl px-3 py-6 sm:px-6 sm:py-9">
      <div className="mb-5 flex items-end justify-between gap-3">
        ...
        <div className="flex shrink-0 items-center gap-2">
          <Link href="/stores" className="inline-flex items-center gap-1.5 rounded-xl border border-border bg-card px-3.5 py-2 text-xs font-bold text-muted-foreground transition hover:border-primary/25 hover:text-primary">{t.viewStores}<ArrowLeft className="h-3.5 w-3.5 rtl:rotate-180" aria-hidden /></Link>
          <Link href="/brands" className="inline-flex items-center gap-1.5 rounded-xl border border-primary/20 bg-primary/5 px-3.5 py-2 text-xs font-bold text-primary transition hover:bg-primary/10"><BadgeCheck className="h-3.5 w-3.5" aria-hidden />{t.brands}</Link>
        </div>
      </div>
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
        {result.items.map((store) => (
          <Link key={store.id} href={`/store/${store.id}` as never} className="group min-w-0 overflow-hidden rounded-2xl border border-border bg-card shadow-sm transition-[transform,box-shadow,border-color] duration-200 hover:-translate-y-0.5 hover:border-primary/25 hover:shadow-md">
            ...
            <div className="p-3">
              <div className="flex items-center gap-2.5"><div className="relative h-10 w-10 shrink-0 overflow-hidden rounded-xl border border-border bg-background">{store.logoUrl ? <Image src={store.logoUrl} alt={store.shopName} fill sizes="40px" className="object-cover" /> : <div className="flex h-full w-full items-center justify-center bg-primary text-xs font-black text-primary-foreground">{store.shopName.charAt(0)}</div>}</div><div className="min-w-0"><h3 className="truncate text-sm font-black group-hover:text-primary">{store.shopName}</h3><span className="mt-1 inline-flex items-center gap-1 text-[9px] font-bold text-emerald-600 dark:text-emerald-400"><ShieldCheck className="h-3 w-3" aria-hidden />{t.verified}</span></div></div>
              <div className="mt-3 flex gap-2 text-[9px] text-muted-foreground"><span className="inline-flex min-w-0 items-center gap-1 rounded-lg border border-border bg-background px-2 py-1.5"><Package className="h-3 w-3 shrink-0" aria-hidden />{store.productCount} {t.products}</span>{store.city ? <span className="inline-flex min-w-0 items-center gap-1 rounded-lg border border-border bg-background px-2 py-1.5"><MapPin className="h-3 w-3 shrink-0" aria-hidden /><span className="truncate">{store.city}</span></span> : null}</div>
            </div>
          </Link>
        ))}
      </div>
...
async function Recommendations({ locale }: { locale: Locale }) {
  ...
  const pool = [...data.featured, ...data.bestSelling, ...data.newest, ...data.popular].filter((product) => !seen.has(product.id) && seen.add(product.id)).slice(0, 20).map((product) => toSliderProduct(product));
