...
import { Compass, Tag, PackagePlus, Store, Sparkles, ArrowLeft, ShieldCheck, MapPin, Package } from 'lucide-react';
...
function copy(locale: Locale) {
  if (locale === 'en') return {
    ...
    title: 'Find your next purchase faster',
    subtitle: 'Explore deals, new arrivals, trusted stores and local products without scrolling through everything.',
    explore: 'Browse all products',
    deals: 'Deals',
    new: 'New arrivals',
    stores: 'Stores',
    local: 'Local products',
    storesTitle: 'Explore trusted stores',
    storesSubtitle: 'Meet verified sellers and open their storefronts directly.',
  ...
  if (locale === 'ps') return {
    eyebrow: 'د بازار کشف',
    title: 'خپل راتلونکی پیرود ژر ومومئ',
    subtitle: 'ځانګړي وړاندیزونه، نوي محصولات، باوري پلورنځي او کورني محصولات په اسانه ومومئ.',
    explore: 'ټول محصولات وګورئ',
    deals: 'ځانګړي وړاندیزونه',
    new: 'نوي محصولات',
    stores: 'پلورنځي',
    local: 'کورني محصولات',
    storesTitle: 'باوري پلورنځي وپلټئ',
    storesSubtitle: 'تایید شوي پلورونکي ومومئ او د هغوی پلورنځي مستقیم خلاص کړئ.',
    viewStores: 'ټول پلورنځي وګورئ',
    verified: 'تایید شوی پلورونکی',
    products: 'محصولات',
  };
  return {
    eyebrow: 'کشف بازار',
    title: 'خرید بعدی‌تان را سریع‌تر پیدا کنید',
    subtitle: 'پیشنهادها، تازه‌واردها، فروشگاه‌های معتبر و محصولات وطنی را بدون گشتن در یک صفحه طولانی پیدا کنید.',
    explore: 'مشاهده همه محصولات',
    deals: 'پیشنهادها',
    new: 'تازه‌واردها',
    stores: 'فروشگاه‌ها',
    local: 'محصولات وطنی',
    storesTitle: 'فروشگاه‌های معتبر را کشف کنید',
    storesSubtitle: 'فروشندگان تأییدشده را ببینید و مستقیم وارد فروشگاهشان شوید.',
    viewStores: 'مشاهده همه فروشگاه‌ها',
    verified: 'فروشنده تأییدشده',
    products: 'محصول',
  };
}
  ...
  return (
    <section id="stores" aria-labelledby="stores-title" className="mx-auto max-w-screen-xl px-3 py-6 sm:px-6 sm:py-9">
      <div className="mb-5 flex items-end justify-between gap-3">
        <div>
          <div className="inline-flex items-center gap-2 rounded-full bg-violet-500/10 px-3 py-1.5 text-[10px] font-extrabold text-violet-600 dark:text-violet-400"><Store className="h-3.5 w-3.5" aria-hidden />{t.stores}</div>
          <h2 id="stores-title" className="mt-3 text-xl font-black tracking-tight sm:text-2xl">{t.storesTitle}</h2>
          <p className="mt-1 text-xs leading-6 text-muted-foreground sm:text-sm">{t.storesSubtitle}</p>
        </div>
        <Link href="/stores" className="inline-flex shrink-0 items-center gap-1.5 rounded-xl border border-border bg-card px-3.5 py-2 text-xs font-bold text-muted-foreground transition hover:border-primary/25 hover:text-primary">{t.viewStores}<ArrowLeft className="h-3.5 w-3.5 rtl:rotate-180" aria-hidden /></Link>
      </div>
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
        {result.items.map((store) => (
          <Link key={store.id} href={`/store/${store.id}` as never} className="group min-w-0 overflow-hidden rounded-2xl border border-border bg-card shadow-sm transition-[transform,box-shadow,border-color] duration-200 hover:-translate-y-0.5 hover:border-primary/25 hover:shadow-md">
            <div className="relative aspect-[2.8/1] overflow-hidden bg-muted">
              {store.bannerUrl ? <Image src={store.bannerUrl} alt="" fill sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 25vw" className="object-cover transition-transform duration-500 group-hover:scale-[1.03]" /> : <div className="h-full w-full bg-gradient-to-br from-violet-500/15 via-muted to-muted" />}
            </div>
            <div className="p-3">
              <div className="flex items-center gap-2.5">
                <div className="relative h-10 w-10 shrink-0 overflow-hidden rounded-xl border border-border bg-background">
                  {store.logoUrl ? <Image src={store.logoUrl} alt={store.shopName} fill sizes="40px" className="object-cover" /> : <div className="flex h-full w-full items-center justify-center bg-primary text-xs font-black text-primary-foreground">{store.shopName.charAt(0)}</div>}
                </div>
                <div className="min-w-0">
                  <h3 className="truncate text-sm font-black group-hover:text-primary">{store.shopName}</h3>
                  <span className="mt-1 inline-flex items-center gap-1 text-[9px] font-bold text-emerald-600 dark:text-emerald-400"><ShieldCheck className="h-3 w-3" aria-hidden />{t.verified}</span>
                </div>
              </div>
              <div className="mt-3 flex gap-2 text-[9px] text-muted-foreground">
                <span className="inline-flex min-w-0 items-center gap-1 rounded-lg border border-border bg-background px-2 py-1.5"><Package className="h-3 w-3 shrink-0" aria-hidden />{store.productCount} {t.products}</span>
                {store.city ? <span className="inline-flex min-w-0 items-center gap-1 rounded-lg border border-border bg-background px-2 py-1.5"><MapPin className="h-3 w-3 shrink-0" aria-hidden /><span className="truncate">{store.city}</span></span> : null}
              </div>
            </div>
          </Link>
        ))}
...
async function Recommendations({ locale }: { locale: Locale }) {
  ...
  const pool = [...data.featured, ...data.bestSelling, ...data.newest, ...data.popular]
