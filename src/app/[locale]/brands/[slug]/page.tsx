...
async function loadBrand(slug: string) {
  ...
  try {
    ...
    const result = await getProductService().listProducts({ sellerId: brand.sellerId, isActive: true, page: 1, pageSize: 24, sort: 'bestSelling' });
    return { status: 'ok' as const, brand, products: result.products };
  } catch (error) {
