...
export default async function SellerDiscountsPage({ params, searchParams }: Props) {
  ...
  const [count, products] = await Promise.all([
    ...
    prisma.product.findMany({
      where,
      select: { id: true, name: true, price: true, compareAtPrice: true, category: { select: { name: true } } },
  ...
  return (
    <Container size="xl" className="space-y-6 py-2">
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        ...
        <div className="rounded-2xl border border-border bg-card p-4"><p className="text-xs text-muted-foreground">صفحه</p><p className="mt-1 text-2xl font-black">{page} / {totalPages}</p></div>
      </div>
      ...
      {products.length === 0 ? (
        <div className="rounded-2xl border border-dashed border-border bg-card p-12 text-center"><BadgePercent className="mx-auto mb-3 h-9 w-9 text-muted-foreground/50" /><p className="text-sm font-semibold">محصول تخفیف‌داری مطابق جست‌وجوی شما پیدا نشد.</p><Link href="/seller/products/new" className="mt-4 inline-flex rounded-xl bg-primary px-4 py-2 text-xs font-bold text-primary-foreground">افزودن محصول</Link></div>
      ) : (
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{products.map((product) => { const oldPrice = product.compareAtPrice ? Number(product.compareAtPrice) : 0; const price = Number(product.price); const pct = oldPrice > price ? Math.round(((oldPrice - price) / oldPrice) * 100) : 0; return <article key={product.id} className="rounded-2xl border border-border bg-card p-4 shadow-sm"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="line-clamp-2 text-sm font-bold">{product.name}</p><p className="mt-1 text-xs text-muted-foreground">{product.category.name}</p></div>{pct > 0 && <span className="shrink-0 rounded-full bg-red-100 px-2 py-1 text-[10px] font-black text-red-700 dark:bg-red-950/30 dark:text-red-300">-{pct}٪</span>}</div><div className="mt-4 flex items-end gap-2"><span className="text-lg font-black text-price-current">{formatPrice(price, 'AFN', locale)}</span><span className="text-xs text-muted-foreground line-through">{formatPrice(oldPrice, 'AFN', locale)}</span></div><Link href="/seller/products/${product.id}/edit" className="mt-4 inline-flex rounded-xl border border-border px-3 py-2 text-xs font-bold hover:bg-muted">ویرایش محصول</Link></article>; })}</div>
      )}
      {totalPages > 1 && <div className="flex items-center justify-between rounded-xl border border-border bg-card px-4 py-3 text-sm"><span className="text-muted-foreground">صفحه {page} از {totalPages}</span><div className="flex gap-2">{page > 1 && <Link href={`?q=${encodeURIComponent(q)}&page=${page - 1}`} className="rounded-lg border border-border px-3 py-1.5">قبلی</Link>}{page < totalPages && <Link href={`?q=${encodeURIComponent(q)}&page=${page + 1}`} className="rounded-lg border border-border px-3 py-1.5">بعدی</Link>}</div></div>}
    </Container>
  );
}
