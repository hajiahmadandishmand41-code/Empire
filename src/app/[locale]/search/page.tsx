...
import { Search, X, Package, ChevronDown, ArrowLeft, Star, RotateCcw } from 'lucide-react';
...
type CategoryRow = { key: string; name?: string; productCount?: number };
...
function SearchCard({ product, text, locale }: { product: Product; text: SearchCopy; locale: Locale }) {
  ...
  return <article className="group relative flex flex-col overflow-hidden rounded-xl border border-border bg-card transition-all hover:-translate-y-0.5 hover:shadow-lg">
    ...
    <Link href={`/shop/${product.slug}`} className="block"><div className="relative aspect-square overflow-hidden bg-muted">{image ? <Image src={image} alt={product.name} fill sizes="(max-width:640px) 50vw, (max-width:1024px) 33vw, 25vw" className="object-cover transition-transform duration-300 group-hover:scale-105" /> : <div className="flex h-full items-center justify-center"><Package className="h-12 w-12 text-muted-foreground/30" /></div>}</div></Link>
...
export default function SearchPage() {
  ...
  const text = useMemo<SearchCopy>(() => ({
    search: translate('search'), searchAria: translate('searchAria'), sort: translate('sort'), category: translate('category'), price: translate('price'), clear: translate('clear'), view: translate('view'), results: translate('results'), resultsFor: translate('resultsFor'), emptyTitle: translate('emptyTitle'), emptyText: translate('emptyText'), noResults: translate('noResults'), noResultsText: translate('noResultsText'), allProducts: translate('allProducts'), allCategories: translate('allCategories'), allPrices: translate('allPrices'), newest: translate('newest'), cheapest: translate('cheapest'), expensive: translate('expensive'), popular: translate('popular'), clearSearch: translate('clearSearch'), wishlist: translate('wishlist'), allShown: translate('allShown'), loading: translate('loading'), error: translate('error'), retry: translate('retry'),
    categories: { clothing: translate('categories.clothing'), digital: translate('categories.digital'), homeAppliances: translate('categories.homeAppliances'), beauty: translate('categories.beauty'), sports: translate('categories.sports'), footwear: translate('categories.footwear'), baby: translate('categories.baby'), books: translate('categories.books'), electronics: translate('categories.electronics'), watches: translate('categories.watches'), traditional: translate('categories.traditional') },
    prices: { low: translate('prices.low'), mid: translate('prices.mid'), high: translate('prices.high'), top: translate('prices.top') },
  }), [translate]);
  ...
  useEffect(() => { fetch('/api/categories', { cache: 'no-store' }).then((response) => response.json()).then((body) => setCategoryRows(Array.isArray(body?.data) ? body.data : [])).catch(() => setCategoryRows([])); }, []);

  const categoryOptions = useMemo(() => {
    const translated = text.categories;
    const dynamic = categoryRows.map((row) => ({ value: row.key, label: translated[row.key] ?? row.name ?? row.key }));
    const known = Object.entries(translated).filter(([key]) => !dynamic.some((row) => row.value === key)).map(([value,label]) => ({ value,label }));
    return [{ value: '', label: text.allCategories }, ...dynamic, ...known];
  }, [categoryRows, text]);
  ...
  const fetchProducts = useCallback(async (requestedPage: number, append: boolean) => {
    ...
    try {
      ...
      const endpoint = isSearch ? '/api/search' : '/api/products';
      const params = new URLSearchParams({ page: String(requestedPage), pageSize: '12', sort: sortApi[sort] });
