    ...
    <div className="mx-auto max-w-[1600px] space-y-5 pb-8">
      <header className="rounded-3xl border border-border bg-card p-5 shadow-sm sm:p-6">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="flex flex-wrap gap-2">
            <Link href="/admin/orders" className="inline-flex min-h-10 items-center gap-2 rounded-xl bg-primary px-4 py-2.5 text-sm font-bold text-primary-foreground">سفارش‌ها <ArrowLeft className="h-4 w-4 rtl:rotate-180" /></Link>
            <Link href="/admin/products" className="inline-flex min-h-10 items-center gap-2 rounded-xl border border-border bg-background px-4 py-2.5 text-sm font-bold">محصولات</Link>
          </div>
        </div>
      </header>
      ...
      <section>
        ...
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          <ActionCard href="/admin/orders?status=pending" icon={<Clock3 className="h-5 w-5" />} value={pendingOrders} title="سفارش‌های در انتظار" note="بررسی سفارش‌های جدید" />
          <ActionCard href="/admin/products?stock=low" icon={<AlertTriangle className="h-5 w-5" />} value={d.lowStockCount} title="موجودی کم" note="محصولات نزدیک به اتمام" />
          <ActionCard href="/admin/sellers?status=pending" icon={<Store className="h-5 w-5" />} value={d.pendingSellers} title="درخواست فروشندگی" note="بررسی و تأیید فروشندگان" />
          <ActionCard href="/admin/products?status=inactive" icon={<Package className="h-5 w-5" />} value={d.inactiveProducts} title="محصول غیرفعال" note="بازبینی وضعیت انتشار" />
        </div>
      </section>
