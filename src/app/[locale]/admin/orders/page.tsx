...
export default async function AdminOrdersPage({ params, searchParams }: Props) {
  ...
  const columns: Column<AdminOrderRow>[] = [
    { key: 'ref', header: labels.number, cell: (r) => <Link href="/admin/orders/${r.id}" className="inline-flex items-center gap-1.5 font-mono text-sm font-bold text-primary hover:underline"><span>{r.reference}</span><ArrowUpLeft className="h-3.5 w-3.5" aria-hidden /></Link> },
  ...
  return <div className="space-y-6">
    <header className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between"><div><div className="mb-2 inline-flex items-center gap-2 rounded-full bg-primary/10 px-3 py-1.5 text-xs font-bold text-primary"><ShoppingBag className="h-3.5 w-3.5" aria-hidden />مرکز عملیات فروش</div><h1 className="font-display text-2xl font-black tracking-tight text-foreground sm:text-3xl">{labels.title}</h1><p className="mt-1 max-w-2xl text-sm leading-6 text-muted-foreground">{labels.subtitle}</p></div><div className="grid grid-cols-2 gap-2 sm:grid-cols-4"><Link href="/admin/orders?status=pending" className="rounded-2xl border border-border bg-card px-4 py-3 transition hover:-translate-y-0.5 hover:shadow-sm"><Clock3 className="mb-2 h-4 w-4 text-amber-600" aria-hidden /><div className="text-xs font-bold">{labels.pending}</div><div className="text-[11px] text-muted-foreground">بررسی سریع</div></Link><Link href="/admin/orders?status=processing" className="rounded-2xl border border-border bg-card px-4 py-3 transition hover:-translate-y-0.5 hover:shadow-sm"><ShoppingBag className="mb-2 h-4 w-4 text-blue-600" aria-hidden /><div className="text-xs font-bold">{labels.processing}</div><div className="text-[11px] text-muted-foreground">در صف آماده‌سازی</div></Link><Link href="/admin/orders?status=shipped" className="rounded-2xl border border-border bg-card px-4 py-3 transition hover:-translate-y-0.5 hover:shadow-sm"><Truck className="mb-2 h-4 w-4 text-violet-600" aria-hidden /><div className="text-xs font-bold">{labels.shipped}</div><div className="text-[11px] text-muted-foreground">در مسیر مشتری</div></Link><Link href="/admin/orders?status=delivered" className="rounded-2xl border border-border bg-card px-4 py-3 transition hover:-translate-y-0.5 hover:shadow-sm"><CreditCard className="mb-2 h-4 w-4 text-emerald-600" aria-hidden /><div className="text-xs font-bold">{labels.delivered}</div><div className="text-[11px] text-muted-foreground">تحویل‌شده</div></Link></div></header>
