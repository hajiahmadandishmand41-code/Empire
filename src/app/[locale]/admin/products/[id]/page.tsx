...
export default async function EditAdminProductPage({ params }: { params: Promise<{ locale: string; id: string }> }) {
  ...
  return <ProductForm
    ...
    initial={{ id: product.id, slug: product.slug, name: product.name, shortDescription: product.shortDescription, description: product.description ?? '', price: product.price.toNumber(), currency: product.currency, categoryId: product.categoryId, region: product.region, badge: product.badge ?? '', inStock: product.inStock }}
