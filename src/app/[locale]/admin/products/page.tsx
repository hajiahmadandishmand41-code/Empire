...
export default async function AdminProductsPage({ params, searchParams }: Props) {
  ...
  const columns: Column<AdminProductRow>[] = [
    ...
    { key: 'actions', header: '', headerClassName: 'w-24', className: 'text-end', cell: (r) => <div className="flex items-center justify-end gap-2"><EditIconButton /><DeleteIconButton endpoint={`/api/admin/products/${r.id}`} confirmMessage={t('empty')} /></div> },
  ];
  ...
  return <div className="space-y-4">
    ...
    <Card className="space-y-4 p-4"><div className="flex flex-wrap items-center gap-3"><SearchForm placeholder={t('search')} />{result.source === 'mock' && <span className="text-xs text-muted-foreground">{t('mock')}</span>}</div>{result.total === 0 ? <EmptyState title={t('empty')} description={t('emptyHint')} /> : <DataTable columns={columns} rows={result.items} rowKey={(r) => r.id} />}<Pagination page={result.page} pageSize={result.pageSize} total={result.total} /></Card>
  </div>;
}
