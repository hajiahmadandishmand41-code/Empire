...
export default async function AdminProductsPage({ params, searchParams }: Props) {
  ...
  const columns: Column<AdminProductRow>[] = [
    ...
    { key: 'actions', header: 'عملیات', headerClassName: 'w-24', className: 'text-end', cell: (r) => <div className="flex items-center justify-end gap-2"><EditIconButton href="/admin/products/${r.id}" label={ta('edit')} /><DeleteIconButton endpoint={`/api/admin/products/${r.id}`} confirmMessage={ta('confirmDelete')} labels={{ deleted: ta('deleted'), failed: ta('deleteFailed'), network: ta('network'), aria: ta('delete') }} /></div> },
  ];

  return <div className="space-y-5">
    <header className="flex flex-col gap-4 rounded-3xl border border-border bg-card p-5 shadow-sm sm:flex-row sm:items-center sm:justify-between"><div><div className="flex items-center gap-3"><div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-primary/10 text-primary"><Package className="h-5 w-5" /></div><div><h2 className="font-display text-2xl font-black text-foreground">مدیریت محصولات</h2><p className="mt-1 text-xs text-muted-foreground">جستجو، بررسی موجودی، مدیریت وضعیت و ویرایش محصولات Marketplace</p></div></div></div><Button asChild size="sm"><Link href="/admin/products/new"><Plus className="h-4 w-4" />{t('add')}</Link></Button></header>
