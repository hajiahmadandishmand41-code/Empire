...
export function MarketplaceProductCard({ product, currency = 'AFN', locale = 'fa', whatsappNumber, view = 'grid', showDescription = false }: MarketplaceProductCardProps) {
  ...
  const imageSizes = view === 'rail' ? '(max-width: 639px) 31vw, (max-width: 1024px) 30vw, 220px' : view === 'list' ? '160px' : '(max-width: 639px) 31vw, (max-width: 1024px) 25vw, 20vw';

  const imageBlock = (
    <div className={cn('relative aspect-[3/2] w-full overflow-hidden bg-muted', view === 'list' && 'aspect-auto h-32 w-32 sm:h-40 sm:w-40')}>
  ...
  return <article className={cn('group relative min-w-0 overflow-hidden rounded-2xl border border-border/65 bg-card shadow-sm transition-[transform,box-shadow,border-color] duration-200 hover:-translate-y-0.5 hover:border-primary/25 hover:shadow-premium', rail ? 'flex w-[calc((100%-1rem)/3)] max-w-[220px] flex-none flex-col sm:w-[calc((100%-1.25rem)/3)] sm:max-w-[230px]' : view === 'list' ? 'flex flex-row' : 'flex h-full min-h-0 flex-col')}>
    <div className={cn('relative', view === 'list' ? 'shrink-0' : 'w-full')}><Link href={`/shop/${product.slug}` as never} className="block">{imageBlock}</Link><WishlistButton slug={product.slug} productId={product.id} size="sm" labelOn={tCard('wishlistLabel')} labelOff={tCard('wishlistLabel')} className={cn('absolute end-1.5 top-1.5 z-10 border border-border/50 bg-card/90 shadow-sm backdrop-blur-sm', rail ? 'h-8 w-8' : 'h-8 w-8')} /></div>
    <div className={cn('min-w-0', view === 'list' ? 'flex flex-1 flex-col justify-center gap-2 p-3 sm:p-4' : rail ? 'flex flex-1 flex-col gap-1 px-2.5 pb-2.5 pt-1.5 sm:px-3' : 'flex flex-1 flex-col gap-1.5 p-2 sm:p-2.5')}>
      <p className={cn('truncate font-bold uppercase tracking-wider text-primary', rail ? 'h-3.5 text-[8px] sm:text-[9px]' : 'h-3.5 text-[9px] sm:h-4 sm:text-[10px]')}>{categoryLabel}</p>
      <h3 className={cn('font-bold leading-tight text-foreground', view === 'list' ? 'line-clamp-2 text-sm sm:text-base' : rail ? 'line-clamp-2 min-h-8 text-[10px] sm:min-h-9 sm:text-[11px]' : 'line-clamp-2 min-h-8 text-[11px] sm:min-h-9 sm:text-sm')}><Link href={`/shop/${product.slug}` as never} className="transition-colors hover:text-primary">{product.name}</Link></h3>
      ...
      <div className="mt-auto flex min-w-0 items-center justify-between gap-1.5 border-t border-border/60 pt-1.5 sm:gap-2">
        <div className="flex min-w-0 flex-col"><span className={cn('truncate text-[8px] text-muted-foreground sm:text-[10px]', originalPrice ? 'line-through' : 'hidden')}>{originalPrice ? formatPrice(originalPrice, currency, locale) : '0'}</span><span className={cn('truncate font-extrabold leading-none', discountPct > 0 ? 'text-price-current' : 'text-foreground', view === 'list' ? 'text-base sm:text-lg' : rail ? 'text-[11px] sm:text-xs' : 'text-[12px] sm:text-sm')}>{formatPrice(product.price, currency, locale)}</span></div>
        <div className="flex shrink-0 items-center gap-1">{waLink ? <a href={waLink} target="_blank" rel="noopener noreferrer" aria-label={`${tCard('whatsappLabel')} — ${product.name}`} className={cn('flex items-center justify-center rounded-full bg-emerald-500 text-white shadow-sm transition hover:-translate-y-0.5 hover:bg-emerald-600 focus-visible:ring-2 focus-visible:ring-emerald-500/50', rail ? 'h-8 w-8' : 'h-8 w-8')}><MessageCircle className="h-3.5 w-3.5" aria-hidden="true" /></a> : null}<button type="button" onClick={handleAddToCart} disabled={!canQuickAdd} aria-label={tCard('addToCart')} className={cn('flex shrink-0 items-center justify-center rounded-full bg-primary text-primary-foreground shadow-sm transition hover:-translate-y-0.5 hover:shadow-md active:scale-95 disabled:cursor-not-allowed disabled:opacity-50 focus-visible:ring-2 focus-visible:ring-primary/50', rail ? 'h-8 w-8' : 'h-8 w-8')}><ShoppingCart className="h-3.5 w-3.5" aria-hidden="true" /><span className="sr-only">{locale === 'en' ? 'Add' : locale === 'ps' ? 'اضافه' : 'افزودن'}</span></button></div>
      </div>
    </div>
  </article>;
}
