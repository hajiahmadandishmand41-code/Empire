...
export function MarketplaceProductCard({ product, currency = 'AFN', locale = 'fa', whatsappNumber, view = 'grid', showDescription = false }: MarketplaceProductCardProps) {
  ...
  const imageBlock = <div className={cn('relative overflow-hidden bg-muted', view === 'rail' ? 'aspect-square w-full' : view === 'list' ? 'h-32 w-32 shrink-0 sm:h-40 sm:w-40' : 'aspect-[4/3] w-full')}>
    {image ? <Image src={image} alt={product.images?.[0]?.alt || product.name} fill sizes={imageSizes} loading={view === 'rail' ? 'lazy' : undefined} className="object-cover transition-transform duration-500 ease-out group-hover:scale-[1.035]" /> : <div className="flex h-full items-center justify-center bg-muted"><ShoppingCart className="h-9 w-9 text-muted-foreground/25" aria-hidden="true" /></div>}
  ...
  return <article className={cn('group relative min-w-0 overflow-hidden rounded-2xl border border-border/70 bg-card shadow-sm transition-[transform,box-shadow,border-color] duration-200 hover:-translate-y-0.5 hover:border-primary/25 hover:shadow-lg', view === 'rail' ? 'flex w-[168px] flex-none flex-col sm:w-[188px] md:w-[204px]' : view === 'list' ? 'flex flex-row' : 'flex h-full flex-col')}>
