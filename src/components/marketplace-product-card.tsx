...
export function MarketplaceProductCard({ product, currency = 'AFN', locale = 'fa', whatsappNumber, view = 'grid', showDescription = false }: MarketplaceProductCardProps) {
  ...
  const imageBlock = <div className={cn('relative overflow-hidden bg-muted', view === 'rail' ? 'aspect-[3/4] w-full' : view === 'list' ? 'h-32 w-32 shrink-0 sm:h-40 sm:w-40' : 'aspect-[4/3] w-full')}>
    {image ? <Image src={image} alt={product.images?.[0]?.alt || product.name} fill sizes={imageSizes} loading="lazy" className={cn('transition-transform duration-500 ease-out group-hover:scale-[1.035]', view === 'rail' ? 'object-cover' : 'object-cover')} /> : <div className="flex h-full items-center justify-center bg-muted"><ShoppingCart className="h-9 w-9 text-muted-foreground/25" aria-hidden="true" /></div>}
  ...
  return <article className={cn('group relative min-w-0 overflow-hidden rounded-2xl border border-border/70 bg-card shadow-sm transition-[transform,box-shadow,border-color] duration-200 hover:-translate-y-0.5 hover:border-primary/25 hover:shadow-lg', rail ? 'flex h-[470px] w-[174px] flex-none flex-col sm:h-[500px] sm:w-[190px] md:h-[535px] md:w-[204px]' : view === 'list' ? 'flex flex-row' : 'flex h-full flex-col')}>
    ...
    <div className={cn('min-w-0', view === 'list' ? 'flex flex-1 flex-col justify-center gap-2 p-3 sm:p-4' : rail ? 'flex min-h-0 flex-1 flex-col gap-1.5 p-3' : 'flex flex-1 flex-col gap-1.5 p-2.5 sm:gap-2 sm:p-4')}>
      <p className={cn('truncate font-bold uppercase tracking-wider text-primary', rail ? 'h-4 text-[9px] sm:text-[10px]' : 'text-[9px] sm:text-[10px]')}>{categoryLabel}</p>
      <h3 className={cn('font-bold leading-snug text-foreground', view === 'list' ? 'line-clamp-2 text-sm sm:text-base' : rail ? 'h-10 line-clamp-2 text-xs font-extrabold leading-5 sm:text-sm' : 'line-clamp-2 text-[12px] sm:text-sm')}><Link href={`/shop/${product.slug}` as never} className="transition-colors hover:text-primary">{product.name}</Link></h3>
      {rail ? (
        product.sellerId && product.sellerShopName ? <Link href={`/store/${product.sellerId}` as never} className="flex h-5 min-w-0 items-center gap-1.5 text-[10px] font-semibold text-muted-foreground transition-colors hover:text-primary sm:text-[11px]"><Store className="h-3 w-3 shrink-0" aria-hidden="true" /><span className="truncate">{product.sellerShopName}</span></Link> : <span className="h-5" aria-hidden="true" />
      ...
      {product.region && <div className={cn('min-w-0 items-center gap-1 text-[10px] text-muted-foreground', rail || view === 'rail' ? 'hidden' : 'flex')}><MapPin className="h-3 w-3 shrink-0" aria-hidden="true" /><span className="truncate">{product.region}</span></div>}
      {rating > 0 ? <div className="flex h-4 items-center gap-0.5" aria-label={`${rating.toFixed(1)}/5`}>{[0,1,2,3,4].map((i) => <Star key={i} className={cn('h-2.5 w-2.5 sm:h-3 sm:w-3', i < Math.round(rating) ? 'fill-price-warning text-price-warning' : 'fill-muted text-muted-foreground')} aria-hidden="true" />)}{reviewCount > 0 && <span className="ms-1 text-[9px] text-muted-foreground sm:text-[10px]">({reviewCount.toLocaleString(locale === 'en' ? 'en-US' : 'fa-IR')})</span>}</div> : rail ? <span className="h-4" aria-hidden="true" /> : null}
