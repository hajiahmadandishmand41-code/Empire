...
export function MarketplaceProductCard({ product, currency = 'AFN', locale = 'fa', whatsappNumber, view = 'grid', showDescription = false }: MarketplaceProductCardProps) {
  ...
  const imageSizes = view === 'rail' ? '(max-width: 639px) 46vw, (max-width: 1024px) 188px, 204px' : view === 'list' ? '160px' : '(max-width: 639px) 50vw, (max-width: 1024px) 33vw, 25vw';

  const imageBlock = (
    <div className={cn('relative overflow-hidden bg-muted', view === 'list' ? 'h-32 w-32 shrink-0 sm:h-40 sm:w-40' : 'aspect-square w-full')}>
  ...
  return <article className={cn('group relative min-w-0 overflow-hidden rounded-2xl border border-border/60 bg-card shadow-sm transition-[transform,box-shadow,border-color] duration-200 hover:-translate-y-0.5 hover:border-primary/25 hover:shadow-md', rail ? 'flex w-[calc((100vw-2.75rem)/2)] max-w-[204px] flex-none flex-col sm:w-[188px] md:w-[204px]' : view === 'list' ? 'flex flex-row' : 'flex h-full min-h-0 flex-col')}>
    ...
    <div className={cn('min-w-0', view === 'list' ? 'flex flex-1 flex-col justify-center gap-2 p-3 sm:p-4' : rail ? 'flex flex-col gap-1.5 p-2.5 sm:p-3' : 'flex flex-1 flex-col gap-1.5 p-3 sm:p-3.5')}>
      <p className="h-4 truncate font-bold text-[9px] uppercase tracking-wider text-primary sm:text-[10px]">{categoryLabel}</p>
      <h3 className={cn('font-bold leading-snug text-foreground', view === 'list' ? 'line-clamp-2 text-sm sm:text-base' : 'line-clamp-2 min-h-9 text-[12px] sm:text-sm')}><Link href={`/shop/${product.slug}` as never} className="transition-colors hover:text-primary">{product.name}</Link></h3>
