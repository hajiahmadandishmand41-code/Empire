...
export function MarketplaceProductCard({ product, currency = 'AFN', locale = 'fa', whatsappNumber, view = 'grid', showDescription = false }: MarketplaceProductCardProps) {
  ...
  return <article className={cn('group relative min-w-0 overflow-hidden rounded-xl border border-border/60 bg-card shadow-sm transition-[transform,box-shadow,border-color] duration-200 hover:-translate-y-0.5 hover:border-primary/25 hover:shadow-md sm:rounded-2xl', rail ? 'flex w-[calc((100vw-2.25rem)/3)] max-w-[204px] flex-none flex-col sm:w-[188px] md:w-[204px]' : view === 'list' ? 'flex flex-row' : 'flex h-full min-h-0 flex-col')}>
    ...
    <div className={cn('min-w-0', view === 'list' ? 'flex flex-1 flex-col justify-center gap-2 p-3 sm:p-4' : rail ? 'flex flex-col gap-1 p-2 sm:p-3' : 'flex flex-1 flex-col gap-1 p-2 sm:p-3')}>
      ...
      <div className="mt-auto flex min-w-0 items-center justify-between gap-1.5 border-t border-border/60 pt-1.5 sm:gap-2 sm:pt-2">
        ...
        <div className="flex shrink-0 items-center gap-1">{waLink ? <a href={waLink} target="_blank" rel="noopener noreferrer" aria-label={`${tCard('whatsappLabel')} — ${product.name}`} className="flex h-7 w-7 items-center justify-center rounded-full bg-emerald-500 text-white shadow-sm transition hover:-translate-y-0.5 hover:bg-emerald-600 sm:h-8 sm:w-8"><MessageCircle className="h-3 w-3 sm:h-3.5 sm:w-3.5" aria-hidden="true" /></a> : null}<button type="button" onClick={handleAddToCart} disabled={!canQuickAdd} aria-label={tCard('addToCart')} className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-primary text-primary-foreground shadow-sm transition hover:-translate-y-0.5 hover:shadow-md active:scale-95 disabled:cursor-not-allowed disabled:opacity-50 sm:h-8 sm:w-8"><ShoppingCart className="h-3 w-3 sm:h-3.5 sm:w-3.5" aria-hidden="true" /><span className="sr-only">{locale === 'en' ? 'Add' : locale === 'ps' ? 'اضافه' : 'افزودن'}</span></button></div>
      </div>
    </div>
  </article>;
}
