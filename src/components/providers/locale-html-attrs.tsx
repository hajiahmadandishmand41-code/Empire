...
export function htmlProps(locale: AppLocale) { return { lang: htmlLangFor(locale), dir: getDirection(locale), suppressHydrationWarning: true } as const; }
...
export async function baseMetadata(locale: AppLocale = 'fa'): Promise<Metadata> {
  ...
  const copy = { title: locale === 'en' ? t('title') : t('title').replace(/Empire\s*Shop|EmpireShop/gi, 'ایشاپ'), description: t('description'), keywords: t.raw('keywords') as string[] };
  const canonicalBase = SITE_URL ? `${SITE_URL}/${locale}` : `/${locale}`;
  return {
    metadataBase: SITE_URL ? new URL(SITE_URL) : undefined,
    title: { default: copy.title, template: `%s | ${brand}` },
    description: copy.description,
    applicationName: brand,
    authors: [{ name: brand, url: SITE_URL }],
    keywords: [brand, ...copy.keywords.filter((keyword) => !/Empire\s*Shop|EmpireShop/i.test(keyword))],
    formatDetection: { email: false, address: false, telephone: false },
    robots: { index: true, follow: true, googleBot: { index: true, follow: true, 'max-video-preview': -1, 'max-image-preview': 'large', 'max-snippet': -1 } },
    openGraph: { type: 'website', siteName: brand, locale: ogLocale, alternateLocale: ['fa_AF', 'ps_AF', 'en_AF'].filter((value) => value !== ogLocale), url: canonicalBase, title: copy.title, description: copy.description, images: [{ url: OG_IMAGE, width: 512, height: 512, alt: brand }] },
    twitter: { card: 'summary', title: copy.title, description: copy.description, images: [OG_IMAGE] },
