...
const TARGETS: Record<ImagePurpose, { maxDimension: number; quality: number }> = { product: { maxDimension: 1600, quality: 0.8 }, logo: { maxDimension: 800, quality: 0.86 }, banner: { maxDimension: 2000, quality: 0.8 } };
...
function loadImage(file: File): Promise<HTMLImageElement> { return new Promise((resolve, reject) => { const url = URL.createObjectURL(file); const image = new Image(); image.onload = () => { URL.revokeObjectURL(url); resolve(image); }; image.onerror = () => { URL.revokeObjectURL(url); reject(new Error('تصویر قابل خواندن نیست.')); }; image.decoding = 'async'; image.src = url; }); }
function canvasBlob(canvas: HTMLCanvasElement, quality: number): Promise<Blob> { return new Promise((resolve, reject) => { canvas.toBlob((blob) => blob ? resolve(blob) : reject(new Error('تبدیل تصویر ناموفق بود.')), 'image/webp', quality); }); }
export async function optimizeImage(file: File, purpose: ImagePurpose = 'product'): Promise<OptimizedImage> {
  ...
  try {
    ...
    const context = canvas.getContext('2d', { alpha: true }); if (!context) throw new Error('مرورگر از پردازش تصویر پشتیبانی نمی‌کند.');
  ...
  } catch { return { file, originalBytes: file.size, optimizedBytes: file.size, width: 0, height: 0, compressionRatio: 1 }; }
}
