...
type UploadResponse = { ok?: unknown; data?: { url?: unknown; size?: unknown; type?: unknown }; url?: unknown; size?: unknown; type?: unknown; error?: unknown };
...
export async function optimizeImage(file: File, purpose: ImagePurpose = 'product'): Promise<OptimizedImage> {
  ...
  for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt += 1) {
    ...
    if (quality > target.minQuality + 0.001) {
      ...
      scale *= 0.82;
      width = Math.max(1, Math.round(sourceWidth * Math.min(1, scale)));
      height = Math.max(1, Math.round(sourceHeight * Math.min(1, scale)));
  ...
  return { file: optimized, originalBytes: file.size, optimizedBytes: optimized.size, width: bestWidth, height: bestHeight, compressionRatio: optimized.size / file.size };
}
...
export function uploadImageWithProgress(url: string, file: File, onProgress?: (percent: number) => void): Promise<{ url: string; size: number; type: string }> {
  return new Promise((resolve, reject) => {
    ...
    xhr.onload = () => {
      ...
      const responseUrl = typeof data?.url === 'string' ? data.url : typeof body.url === 'string' ? body.url : null;
      if (xhr.status >= 200 && xhr.status < 300 && responseUrl) {
        resolve({ url: responseUrl, size: typeof data?.size === 'number' ? data.size : typeof body.size === 'number' ? body.size : file.size, type: typeof data?.type === 'string' ? data.type : typeof body.type === 'string' ? body.type : file.type });
        return;
      }
      reject(new Error(typeof body.error === 'string' ? body.error : typeof body.error === 'object' && body.error && 'message' in body.error ? String((body.error as {message?: unknown}).message ?? '') : 'آپلود تصویر ناموفق بود.'));
    };
