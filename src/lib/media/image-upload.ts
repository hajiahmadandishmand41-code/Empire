...
/** Browser MIME is advisory. Unknown filename/suffixes must still reach byte validation. */
export function isSupportedImageType(mime:string):boolean{
  if(mime===''||mime==='application/octet-stream')return true;
  if(mime.startsWith('video/')||mime==='application/pdf'||mime.startsWith('audio/'))return false;
  return true;
}
...
export function imageUploadError(file:File):string|null{
 ...
 if(file.type&& !isSupportedImageType(file.type))return'فقط فایل تصویری معتبر مجاز است؛ نام و پسوند فایل مستقل از نوع واقعی تصویر است.';
 return null;
}
