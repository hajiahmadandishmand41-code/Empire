...
const signatures: Record<string, Buffer> = { 'image/jpeg': jpeg, 'image/png': png, 'image/webp': webp, 'image/gif': gif, 'image/avif': ftyp('avif', 'mif1'), 'image/bmp': bmp, 'image/tiff': tiff, 'image/x-icon': ico, 'image/vnd.microsoft.icon': ico, 'image/heic': ftyp('heic', 'mif1'), 'image/heif': ftyp('heix', 'mif1'), 'image/jxl': jxl, 'image/apng': png, 'image/svg+xml': svg };
...
describe('image upload contract', () => {
  ...
  it('detects common signatures independently of filename', () => { expect(detectImageMime(jpeg)).toBe('image/jpeg'); expect(detectImageMime(png)).toBe('image/png'); expect(detectImageMime(webp)).toBe('image/webp'); expect(detectImageMime(gif)).toBe('image/gif'); expect(detectImageMime(ftyp('avif','mif1'))).toBe('image/avif'); expect(detectImageMime(ftyp('heic','mif1'))).toBe('image/heif'); expect(detectImageMime(ftyp('heix','mif1'))).toBe('image/heif'); });
  ...
  it('does not confuse MP4 with HEIC/HEIF/AVIF', () => { const mp4 = ftyp('isom','isom'); expect(hasValidImageSignature(mp4,'image/heic')).toBe(false); expect(hasValidImageSignature(mp4,'image/heif')).toBe(false); expect(hasValidImageSignature(mp4,'image/avif')).toBe(false); expect(detectImageMime(mp4)).toBeNull(); });
